...
1. **当前最紧的阻断是「路由过的会话无法重开」**（`boat/bundles/run/tests/reopen.composite.ts:109-113`），解决它不需要改内核。skill-router 不再写自有节点 `boat/skill-routed`，改为用一条带来源的持久 user/message 记录激活。这一点有意偏离蓝图 §9 的「路由结果走 E1 ignorable」。原因是 dsh 规定 ignorable 只能用于「丢失后不影响重建」的记录（`dsh/core/session/src/types.ts:500-511`），而当前激活的技能决定了会话重开后的工具面。
...
7. **需要新增 `boat/lib` 层**，因为现行分层下有两类东西没有合规位置：
...
**内核包。** boat 接管了 13 个 dsh 包（`dsh/kernel.json`）：
...
**内核当前的 carry。** 登记了两项 extend（`compatibility/contract/extensions.yml`），分别来自提交 92698ff 和 4870926。拿 boat 的 agent-loop 与上游 rc.1 做 diff，实测上游文件里多出以下内容：
...
| G2 上游测试 | 上游为内核包写的测试原样在 boat 源码上通过 |
| G3 跨包测试 | 依赖内核的官方包，在 boat 内核上仍能通过自己的测试 |
| persistence / typert | 持久化 schema 和发布出去的 Typert 文件，与上游生成器的产物一致 |
| G4 日志等价 | 同一个脚本化运行，官方版和 boat 写出的会话日志逐事件相同 |
| G5 社区金丝雀 | 钉版的社区插件在两边行为相同，并且绑定到 boat 内核 |
...
`CLAUDE.md:69` 规定了新行为的落点顺序：先考虑 boat 插件，再考虑 seam provider，然后是 boat 自有 seam，最后才轮到内核改动。内核改动算设计决策，设计文档里必须写清两件事：为什么不能放在内核外，以及它属于蓝图 §7 的哪个变更类别。

dsh 目前不接受外部 PR（`dsh:CONTRIBUTING.md:9`）。所以 boat 带进内核的改动很可能要长期携带，每一行 carry 在每次同步时都有成本。
...
|---|---|---|---|---|
| llm | `dsh/llm/llm`（内核） | dsh-base 的 `llm`、`llm-deepseek`、`llm-pi-ai` 行（`dsh:packages/bundle/base/cordis.patch.yml:34,127,524`） | 已有 | 新增 `@boat/llm-openai-compat` 行，用于业务模型 |
...
15. **上游测试从不修改。** boat 自己的测试放在 `tests/boat/` 下（`CLAUDE.md:72`）。
...
**字段说明**
- **建议**：引入；部分（改形态后引入）；不引入。
- **落点**：内核 extend；dsh 原样（直接用 dsh 包，只做配置）；boat 插件；boat seam；boat lib；bundle；agent 层；不移植。
...
1. **准入函数的登记。** `@boat/intake-guard` 发布宿主服务 `boatIntake`，agent 行调用 `register(presetId, gate)` 登记本 agent 的准入函数，返回 disposer。准入函数负责：
   ...
   - LLM 分类；
   - 门槛判定，并渲染门槛卡。
2. **在 followup 之前调用。** `@boat/agent-invoke` 先调用准入函数，再把结果写进本条用户消息 source 的 `boatRequest` 字段：
   - `context`：白名单里的请求字段；
   - `intake`：决定、判定、帧类型与数据、卡片。
3. **循环内只读取结果。** boat/intake 的监听者只在 `step === 1`、并且消息是人类输入时，读取 claimed 消息上的 `intake`：结论是 reply 就回复，是 pass 就放行。
...
- 技能正文改为只在激活时追加一次持久消息；
- `boat:state` 只渲染声明为模型可见的键，并加字节预算。
...
4. **Workflow 移植。** 移植为 `@boat/flow-fsm`，名字避开 dsh 的 workflow。
...
**建议。**
- **`@boat/llm-openai-compat`。** 直接用 dsh-llm-pi-ai 依赖的 pi-ai 库（`dsh:packages/llm/llm-pi-ai/package.json:44`）：
...
- **「只有 loop 知道本步是不是首步」。** `BoatStepPayload` 已经带了 turn 和 step（`dsh/core/agent-loop/src/boat/step-hooks.ts:35-42`）。
...
**从方案三引入：**
- 纯函数 `boatRunOutcomeOf`；
- 设了进入条件的 `boat/lib` 层；
- seam 使用清单脚本，列出每个 boat 包用到了哪些 dsh 事件和服务；
...
flowchart TB
  subgraph L6["L6 apps"]
    CLI["@boat/cli<br/>run · web · serve · eval · plugin · agent new<br/>启动后审计五项必备服务"]
  end
  subgraph L5["L5 bundles：组合即数据"]
    HOST["@boat/host<br/>boat 服务行 · 关闭遥测上传<br/>常驻 memory-store-local"]
    BIZ["@boat/biz<br/>业务工具面 · 关 PTC 与编码工具<br/>业务压缩引擎 · time-context"]
    SERVE["@boat/serve<br/>复用 webserver 行 · /chat 同步与 SSE · 鉴权"]
    RUNEVAL["@boat/run · @boat/eval"]
  end
  subgraph L4["L4 agents：每个业务 agent 一个 dsh bundle"]
    AG["preset-id 声明行<br/>子行：策略行 + ./lib/agent.js<br/>准入函数 · 数据层单例 · 常驻进程"]
  end
  subgraph L3["L3 boat 插件"]
  ...
  subgraph L2["L2 @boat/contracts：类型、常量、声明合并"]
    CT["服务声明 · source 声明 · BoatToolMeta<br/>BOAT_INFORMATIONAL_EVENTS"]
  end
  ...
  subgraph L0["L0 内核 dsh/：13 包"]
    K1["agent-loop<br/>extend：boat/intake · boat/pre-assemble，冻结"]
...
| 层 | 目录 | 放什么 | 可以依赖 |
|---|---|---|---|
| L6 | `boat/apps/*` | 进程与启动器 | bundles、plugins、lib、core |
| L5 | `boat/bundles/*` | 组合：只连线，不实现行为 | plugins、lib、core |
| L4 | `boat/agents/*` | 业务 agent 的 bundle 与领域代码 | plugins（源码只允许 import type 或 inject）、lib、core |
| L3 | `boat/plugins/*` | dsh seam 与 boat seam 的 provider，以及策略插件 | plugins（只允许 import type）、lib、core |
| L2b | `boat/lib/*`（新增） | seam 抽象基类、被三个以上包复用的纯函数与调用助手 | lib、core |
| L2 | `boat/core/*` | contracts（只放类型、常量和声明合并）、cordis-compat | core |
| L1 | npm 上的 dsh 包 | 原样使用 | 无 |
| L0 | `dsh/` | 13 个内核包，不依赖任何 boat 包 | 无 |
...
1. **已有信封（默认档）：**
   - 带来源的 user/message，来源包括 `boat-request`、`plugin:boat-skill-activation`、`plugin:boat-memory`、`plugin:boat-history-import`；
   - `tool/result.meta.boat`；
   - 回复消息的 `source.provider='boat'`。
2. **E1 ignorable：** 只允许纯信息记录，即 `boat/route-request`、`boat/aux-llm-request`、`boat/aux-llm-result`。
3. **发行版必需词表（E1b）：** 目前为空。今后要加任何词都需要拍板，而且官方 dsh 会拒读含这类事件的会话。

#### `boat-request` 这个 source 怎么设计
...
|---|---|---|---|---|
| dsh-agent-loop | `boat/intake` 与 `boat/pre-assemble` 两个 waterfall，以及 reply 分支 | extend（已登记） | 保持并冻结；补 inert 测试；标注锚点函数 `Agent#preStep` | `agent/pre-step` 在 assemble 之后才派发（`dsh/core/agent-loop/src/agent.ts:290-299`），插件在那里既不能「不发模型请求就直接回复」，也无法改变同一步的 prompt |
...
| dsh-tools | 不改；蓝图 §9 的条件 extend 不触发 | 无 | 无 | PTC 下 presentationMeta 会丢，由 `@boat/biz` 关掉 PTC 来解决 |
...
| dsh-agent-loop（候选） | 放宽 `BoatIntakeReply`，允许替换 claimed 消息（约 3 行） | extend（扩展已登记的接口） | 待拍板 | 只有这样，boat web 路径上的拒识轮才能记下判定和卡片 |
| dsh-session（E1b） | 发行版必需词表 | 候选 extend | 目前没有需求 | 官方 dsh 会拒读含这类事件的会话 |
| dsh-session-persistence | 不改；SQL 作为第二个 provider；契约套件复制一份到 `@boat/testing` | 无 | 阶段 D | seam 是抽象类，provider 在内核外 extends 它即可 |

#### boat 层新包清单

| 包 | 层 | 类型 | 职责 | 阶段 |
|---|---|---|---|---|
| `@boat/turn-outcome` | lib | 纯函数 | `boatRunOutcomeOf`，结果分 completed、tool_stopped、rejected、stopped_by_limit、errored、cancelled | B |
| `@boat/agent-invoke` | lib | 调用助手 | 打开 agent；调用准入函数；按请求 id 提交一轮并收口；从 `boat/bundles/run/src/index.ts:184-217` 提取 | B |
| `@boat/memory-store`、`@boat/lease`、`@boat/session-directory` | lib | 抽象基类加 conformance 套件 | 三个 seam 的定义 | B（memory-store、session-directory），D（lease） |
| `@boat/memory-store-local` | plugins | provider | 基于 storage-domain 的单进程记忆存储，由 host 常驻挂载 | B |
| `@boat/biz`、`@boat/serve`、`@boat/eval` | bundles | 组合 | 业务收紧、对外服务、评测 | A / B / C |
| `@boat/request-context` | plugins | 宿主服务 | `boat-request` 这个 source、白名单渲染、把凭证放进请求作用域的 WeakMap | B |
...
| `@boat/session-persistence-sql`、`@boat/datasource-sql`、各 seam 的 sql provider、`@boat/telemetry-traces` | plugins | provider | 多实例与观测 | D |
| `@boat/proactive`、通知存储、用户目录 | plugins | 能力与 provider | 主动服务 | E |

**需要改动的现有包：**
- `@boat/tool-policy`：修 prune 重折；按键声明合并语义；声明模型可见的键；加上 `defaultVisibility`、allow 掩码和 register 辅助函数。
- `@boat/skill-router`：激活改为写持久消息；删除 `boat/skill-routed`；支持 `router:false`；现有三处 warn 改为直接报错。
- `@boat/a2ui`：补 blocks 模式和延迟出卡；说卡交错放在包内；提供 `cardMeta` 和 client 子路径。
- `@boat/history-import`：支持增量合并。
...
| `@boat/step-budget` | max_turns 与 stopped_by_limit |
| `@boat/turn-review` | before_loop_end 的 RETRY |
| tool-policy 加 `concludeTurn` | loop_action=STOP；单工具超时；并行执行；visibility；state_delta；入参纠正 |
| `@boat/request-context` | context_updates；state 的 user: 与 temp: 命名空间；trace_id |
| `@boat/agent-invoke` 加 `@boat/session-directory` | invoker；resolve_session（id 由调用方指定）；按用户归属 |
| `@boat/session-persistence-sql`、`@boat/datasource-sql` | SQL 会话；运行锁；seq 分配；方言；迁移；DDL 导出 |
...
| `@boat/suggestion`、`@boat/citation` | SuggestionGenerator；citation_batch |
| `@boat/telemetry-traces`、`@boat/eval` | OTel 追踪；Evals |
| `@boat/proactive` 加通知存储 | @proactive；notifications 插件 |
...
| eval_seed_cases.py | 放到 `agents/yinglong/eval/` 下，作为 `@boat/eval` 的种子 | grader 读日志、投影和推导出的帧 |

迁移后的包结构：

```
boat/agents/yinglong/
  ...
  skills/               4 个技能，kebab 名，metadata.boat.requiredTools
...
所有 reply 分支的结果（话术、帧、卡片、判定）都写进本条用户消息的 `boatRequest.intake`。循环里的 boat/intake 只负责读取和回复。
...
sequenceDiagram
    ...
    INV->>AL: followup 一条 boat-request 用户消息（requestId，traceId，context，intake）
    AL->>GD: boat/intake（step 1，人类输入）
    alt intake 为 reply（拒识、转人工或门槛卡）
        GD-->>AL: reply 预先算好的话术
        AL-->>INV: assistant/message（provider 为 boat），turn/end completed
        INV-->>W: boatRunOutcomeOf 为 rejected，卡片与帧取自用户消息 source
        W-->>SA: 文本帧，intake_check 或卡片帧，多轮结束标记
    else intake 为 pass
        GD-->>AL: pass
        AL->>SR: boat/pre-assemble
        ...
        T-->>AL: tool/result，meta.boat 带 stateDelta 与 digestPast
        AL->>M: 第 2 步
        M-->>AL: 调用 render_a2ui（综合诊断卡，deferred_discard）
        AL->>T: 渲染模板
        T-->>AL: tool/result，meta.boat 带卡片
        AL->>M: 第 3 步
        M-->>AL: 终答正文，含卡片区域标记
        AL->>AL: agent/turn-stopping，turn/end completed
        AL-->>INV: 按 requestId 收口本轮
        INV-->>W: boatRunOutcomeOf 为 completed
...
**验收**
- `reopen.composite.ts` 里 routed 用例的断言反转：`refusal` 为 undefined，事件类型里不再出现 `boat/` 前缀；boat web 能打开这个会话并续一轮。
- prune 回归测试：先写 x=1，再写 x=2，然后强制 prune 最早的那条 tool/result，boatState.x 仍是 2，卡片不重复也不回退。
...
| dsh 的信任模型只有单一操作者；session-query 不做授权 | 终端用户鉴权和多租户隔离的责任在 boat | serve 默认要求网关共享密钥或签名；resume 之前校验归属；session-query 不对外暴露 |
| SQL provider 的写放大：checkpoint-policy 在每次模型请求前、每个顶层工具执行前都会 flush | 容量和延迟 | 阶段 D 之前出容量估算；配合 projection-cache |
| prune 重折缺陷（推断） | 上下文压力大时 boatState 回退，卡片重复 | 阶段 A 修复，并加回归测试 |
| `@boat/biz` 关掉了 jobs、goals 等服务，依赖它们的社区插件会一直等待，不会报错 | 这些社区插件在业务 profile 里用不了 | 写进 COMPAT.md；`plugin add` 时比对插件的 inject 清单与 biz 关掉的行，给出提示 |
| G6 对自有 source 的覆盖不足 | 官方 dsh 续写后，boat 的投影可能不一致 | 阶段 B 新增往返变体测试 |
