...
> 贯穿全文的例子是「保单查询助手」（agent id `policy-desk`）。它不在仓库里：照本文逐字建出来之后，`pnpm run lint`、`pnpm run typecheck`、`pnpm run test` 全部通过（§2.17），文中的命令输出和日志片段都来自在构建好的二进制上用脚本模型的真实运行，过长处注明了裁剪，本机路径换成了占位符。这些运行录于一次性模式改名为 `lyteboat try`（原名 `lyteboat headless`）和 agent 有了身份之前：命令、包名和路径已按新名字写出；现在的人类消息在 `source.lyteboatRequest` 里还多一个 `agent`（agent 的身份，§4.16），录下的片段里没有它，其余输出不受影响。§4.16 的输出是在现在的构建上跑的。
...
| `state_delta` | `ctx.toolPolicy.register(def, { stateDelta })` → 日志里的 `tool/result.meta.lyteboat.stateDelta` → `lyteboatState` 投影。参考实现的 `output_state_keys`（声明并校验状态键）没有对应物：`LyteboatToolMeta` 只有 `visibility`、`stateDelta` 两个字段（`lyteboat/core/contracts/src/index.ts:106-115`） |
...
加载 agent 目录的有六处：`lyteboat try`、`lyteboat serve`（§4.13）、`lyteboat eval`（§4.14）、`lyteboat web`（§4.15）、`lyteboat studio`（§4.17），以及 composite 测试里的 `bootComposition` / `startComposition`（§2.15）。`lyteboat try` 是一次性的：一个任务、一轮；这一轮打到 stdout（回答原文，每张卡片各占一行 `[card <区域>]`），会话 id 打到 stderr（`lyteboat: session <id>`），然后退出（`lyteboat/bundles/try/src/index.ts:276-283`）。
...
- **也可以导入外部历史。** 用 `--history <file>` 把外部系统的几轮对话导入成一个新会话的已结束轮次，再跑一轮，例如 `node lyteboat/apps/cli/lib/bin.js try --agents ./examples/agents --agent finance --context '{"customer":"young-idle-cash"}' --history lyteboat/bundles/try/tests/fixtures/history/rounds.json "继续刚才的话题"`（`CLAUDE.md`「Commands」，`lyteboat/bundles/try/src/index.ts:249-254`）。`--history` 只能开新会话，不能和 `--session-id` 一起用（`lyteboat/bundles/try/src/startup.ts:123`）。
...
| `!!js` | 行配置里的 YAML 标签，值是加载时求值的 JS 表达式 | `task: !!js ctx.lyteboatTryStartup.task`（`lyteboat/bundles/try/cordis.patch.yml:34`） |
...
| profile | 按顺序列出的 bundle 层 | `try` = dsh-base + `@lyteboat/host` + `@lyteboat/business-base` + `@lyteboat/try`（`lyteboat/apps/cli/src/templates.ts:18-20`）；`serve` 的最后一层换成 `@lyteboat/serve`（`:24-26`），`eval` 的换成 `@lyteboat/eval`（`:27-29`），`studio` 的换成 `@lyteboat/studio`（`:30-32`）；`web` = dsh-base + `@lyteboat/host` + dsh-web-app + `@lyteboat/web`（`:21-23`） |
...
- **目录就是 `baseUrl`。** `@lyteboat/agent-catalog` 先用 `readAgentDefinition` 读出行和清单（显示字段、版本、模型），算出目录的摘要，查过发布锁的钉和声明的模型（组合打开了这两项时，§4.16），再以目录的 `file:` URL 作为 `baseUrl` 登记（`lyteboat/plugins/agent-catalog/src/index.ts:235-270`）。登记之前被拒的 agent，它的代码不会运行。所以 `./lib/tools.js` 这样的相对行名、行配置里的相对路径，都相对 agent 目录解析。preset 注册表本身是 try patch 插入的另一个 dsh 行，不属于 `@lyteboat/try`（`lyteboat/bundles/try/cordis.patch.yml:24-28`）。
...
- **准入在循环之外。** `@lyteboat/try` 把任务交给 Agent 时用的是 `ctx.intakeGuard.submit`：它问这个 agent 登记的准入函数（作用域链上最近的那个），把判定和任务写进同一条人类消息，再 `followup` 给 Agent（`lyteboat/bundles/try/src/index.ts:271`，`lyteboat/plugins/intake-guard/src/index.ts:110-133`）。循环里 intake-guard 只按记录作答；一条没有经过 `submit` 就进来的消息（调用方直接 `agent.followup`，或者 `/chat`、`lyteboat web` 经 session-controller 送进来的消息，§4.13、§4.15），它在循环里补做一次准入，回复相同，只是判定和判定里的卡片不记在消息上（同文件 :1-13、:136-143）。

### 1.3 每个文件、每个字段

**`agent.cordis.yml`（必需）。** 一个 Cordis 行列表，每行是 `id`、`name`、`config`，允许 `!!js` 标签。`@lyteboat/agent-catalog` 用 `entryListSchema` 读它，只检查「是不是列表」，原样交给 registry 作为 preset 的 `plugins`，每一行由 registry 校验（`lyteboat/plugins/agent-catalog/src/agent-directory.ts:121-138`）。目录里有这个文件才算 agent（`agent-directory.ts:26`、`:34-36`）。
...
| `@lyteboat/tool-policy/agent` | 给别的行注册的工具（例如 dsh 官方工具）声明策略，并决定没有声明点名的继承工具是否可见 | `inherited`（`visible` / `hidden`，省略时为 `visible`）、`tools: { <name>: { visibility? } }`，行和工具策略里的未知键（包括去掉了的 `undeclared`、`requiresConfirmation`）加载时直接抛错；`stateDelta` 只能在代码里声明（`lyteboat/plugins/tool-policy/src/agent.ts:32-67`）。用法见 §4.11 |
...
**`agent.yml`（可选）。** agent 的清单（contracts 的 `LyteboatAgentManifest`，`lyteboat/core/contracts/src/index.ts:252-295`），每个字段都可选：
...
按严格 schema 读：多一个键、版本不是这个形状、字段类型不对，加载就失败并指出键名，例如 `agent-catalog: <目录>/agent.yml: (the file): Unrecognized key: "color"`、`… version: must look like 1.2.3 or 1.2.3-rc.1`。原来的 `preset.yml` 已经换成它：目录里还留着 `preset.yml`，加载就失败，提示改名：`agent-catalog: <目录>/preset.yml is now agent.yml: rename it (name, description, and order stay; version and model are new)`（`agent-directory.ts:91-103`）。
...
1. `try` profile 依次加载 `@deepseek-ai/dsh-base`、`@lyteboat/host`、`@lyteboat/business-base`、`@lyteboat/try` 四层（`lyteboat/apps/cli/src/templates.ts:18-20`）。profile 跳过了模板列出的任何一个 bundle，启动就失败（`lyteboat/apps/cli/src/profile-boot.ts:146-155`）。
...
3. `@lyteboat/try/startup` 解析参数。没有任务、`--agents` 目录不存在、`--agent` 和 `--agents` 只给了一个、id 找不到、`--session-id` 为空或和 `--history` 同给、`--context` 不是 JSON 对象，都按用法错误退出，找不到 id 时列出可用 id（`lyteboat/bundles/try/src/startup.ts:103-131`）。
4. `agent-catalog` 读目录（行、清单、摘要）、登记 preset，runner 等它就绪，取出这个 agent 的身份，把会话的工作目录定为这个 agent 的 `$LYTEBOAT_HOME/agent-workdirs/<id>`，不存在就建出来（`lyteboat/bundles/try/src/index.ts:232-241`），按 `agentDefaultModel` 的当前选择创建 Agent，给了 `--session-id` 时改为从日志恢复那个会话（`:226`、`:256-265`），在 `setup` 里 `presets.mount(agentCtx, id)`（`:242-246`）。上游 `mountPreset` 加载各行、审计、拒绝失败的行和泄漏到根 realm 的服务（`mount.ts:258-272`）。
5. runner 用 `intakeGuard.submit` 提交任务（准入，再把任务连同发起者 `operator:cli`、agent 的身份、请求上下文和判定写成一条用户消息交给 Agent），等到空闲，`sessions.flush`；然后用 `a2ui.turnParts` 把这一轮排成回答加卡片打到 stdout，把 `lyteboat: session <id>` 打到 stderr；只有本轮以 `completed` 结束才退出 0（`index.ts:266-283`）。
...
- **`policy-desk-tool-surface`：只露出自己声明的工具。** 业务模式（try、serve、eval）本来就只给 agent dsh-base 的 `skill` 工具，编码工具都被业务底座关掉了（§4.11）；可同一个 agent 在 `lyteboat web` 里会继承 dsh 的全套编码工具，一个 `--plugin` 文件也能往宿主上加工具。`inherited: hidden` 让这个 agent 继承来、却没有任何声明点名的工具一律隐藏，而且再也激活不了（`lyteboat/plugins/tool-policy/src/agent.ts:1-17`）；`skill` 在 `tools` 里声明成 `always`，留着可见；本 agent 用 `ctx.toolPolicy.register` 注册的两个工具算声明过的，照常由技能激活。金融智能体就这样写（`examples/agents/finance/agent.cordis.yml:31-36`）。
...
这三个字段只用于显示；`order` 接着金融智能体的 1 往下排（`examples/agents/finance/agent.yml:3`）。`version` 和 `model` 这里先不写：它们是发布用的，§4.16 讲什么时候加、加了之后要做什么。金融智能体两个都写了（`examples/agents/finance/agent.yml:4-5`）：`version: 1.0.0`，`model: { provider: deepseek-official, model: deepseek-flash }`。
...
`policySummary` 故意去掉了 `insured` 和 `premium`。原因在机制上：状态增量原样持久化在会话日志里（`tool/result.meta.lyteboat.stateDelta`），折成 `lyteboatState` 以后每一步都作为 `lyteboat:state` runtime context 重新发给模型（`lyteboat/plugins/tool-policy/src/index.ts:109-117`，§4.8）。所以写进状态的东西要少，也不能带个人信息。
...
- 顶层的 `surfaceId` 只是占位。渲染时换成 `<card>-<sessionId 前 8 个字符>-<6 位 hex>`（`lyteboat/plugins/a2ui/src/engine.ts:98`、`:125-127`）。lyteboat 的会话 id 总是 `session-<uuid>`（`lyteboat/bundles/try/src/index.ts:260`），前 8 个字符恰好是 `session-`，所以实际的 surfaceId 形如 `policy_card-session--747561`，中间是两个连字符。
...
- **`ctx.toolPolicy.register(definition, meta)`**：在调用方作用域的层里注册工具，同时登记 lyteboat 元数据，返回一个同时撤销两者的 disposer（`lyteboat/plugins/tool-policy/src/index.ts:134-148`）。注册过的工具算声明过的，`inherited: hidden` 不藏它。`meta` 只有两个字段：
  - `visibility: 'auto'`：技能被路由到之前对模型隐藏。每次 `lyteboat/pre-assemble` 结束后，tool-policy 重新计算本 agent 的 `restrict({ deny })`（`index.ts:121-124`、`:257-293`）。
  - `stateDelta: (args, value) => …`：tool-policy 把它包进 `output.presentationMeta`，于是结果 meta 带上 `{ lyteboat: { stateDelta } }`，并和工具自己的 `lyteboat.cards` 合并（`tool-policy/src/index.ts:72-92`）。
...
- **`defineTool`**（`dsh/core/tools/src/schema.ts:483-554`）：
  ...
  - `output.presentationMeta(args, value)` 产出**持久化但模型看不到**的 meta。它只对模型直接发起的调用计算（`dsh/core/tools/src/index.ts:1845-1853`）。带 `exec.parent` 的调用不计算：那是 `run_code`（PTC 模式）的 SDK 发出的子调度（`index.ts:340-349`）。所以在打开 PTC 的组合里，经 `run_code` 调到的 `query_policy` 既不出卡也不写状态；业务底座关掉了 PTC（`ptc-runtime`、`workflow-ptc`），try、serve、eval 里没有这个问题。子 agent 里的工具调用对子 agent 来说是模型直接调用，会计算 meta，但写进子会话自己的日志，不会折进父会话的 `lyteboatState`。
...
- **出卡的三个函数都来自 `@lyteboat/a2ui`：**
  ...
  - `cardsPresentationMeta(cards)` 把卡片数组放成 `{ lyteboat: { cards } }`；没有卡时返回 `{}`，不留空信封；卡片不合 schema 时抛错，这次调用失败，而不是让日志留下一个投影拒收的信封（`lyteboat/plugins/a2ui/src/cards-projection.ts:29-37`）。`lyteboatCards` 投影、`turnParts` 和 `lyteboat try` 的输出都只读 `meta.lyteboat.cards` 这个数组（`cards-projection.ts:52-65`）。
...
- **准入函数在请求进入循环之前运行。** `lyteboat try` 用 `ctx.intakeGuard.submit(agent, { text, context, owner, agent }, signal)` 提交任务（`owner` 是 `{ kind: 'operator', id: 'cli' }`，`agent` 是 agent-catalog 条目里的身份，`lyteboat/bundles/try/src/index.ts:271`）：`submit` 找到这个 agent 作用域链上最近登记的准入函数，调用 `admit({ agent, text, context, signal })`，给判定补上 `by`（准入函数的 `name`），再用 `requestContext.message(...)` 把任务、发起者、agent 的身份、请求上下文和判定写成一条人类消息 `followup` 给 Agent（`lyteboat/plugins/intake-guard/src/index.ts:92-98`、`:110-133`，`lyteboat/plugins/request-context/src/index.ts:48-53`）。放行的请求也带判定，日志里是 `"lyteboatRequest":{"owner":{"kind":"operator","id":"cli"},"intake":{"decision":"pass","by":"policy-desk-intake"}}`（§3.2 的 seq 6，录于 agent 有身份之前；现在 `owner` 后面还有 `"agent":{"id":"policy-desk","digest":"sha256:…"}`）。`submit` 返回记下的判定；没有登记准入函数时返回 `undefined`，消息上也不带判定。
- **判定的字段**（`lyteboat/core/contracts/src/index.ts:213-222`）：`decision` 为 `'pass'`（交给模型）或 `'reply'`（直接作答）；`verdict` 是 agent 自己的标签，例如 `out_of_scope`，只用于审计；`text` 是回复正文；`cards` 是随回复出的卡，每张同样是 `{ surfaceId, area, emission, payload }`，用 `ctx.a2ui.renderCard` 渲染（金融智能体的未授权卡就是这样出的，`examples/agents/finance/src/intake/finance-admission.ts:119`）。`by` 由 intake-guard 填，函数不用返回。
- **`reply` 怎么变成回答。** 循环第一步派发 `lyteboat/intake` 时，intake-guard 的监听先 `await next()`，让排在它里面的 `lyteboat/intake` 监听（例如某个 agent 行自己的拒识门）先判定；它们都放行，它才读人类消息上记录的判定，把 `reply` 变成 `{ kind: 'reply', plugin: by, content: [{ type: 'text', text }] }`（`lyteboat/plugins/intake-guard/src/index.ts:69-75`）。内核把这一步写成一条不请求模型的助手消息，`source` 为 `{ kind: 'model', provider: 'lyteboat', model: 'policy-desk-intake' }`（`dsh/core/agent-loop/src/lyteboat/intake-reply.ts:32-58`，`src/lyteboat/step-hooks.ts:16`）。判定里的卡片不进助手消息：`lyteboatCards` 投影和 `turnParts` 从人类消息上的判定读它们（`lyteboat/plugins/a2ui/src/cards-projection.ts:45-50`、`:57-65`），`lyteboat try` 把它们和回复一起打出来。
- **只有第一步做准入**，工具之后的续步不做判定（`lyteboat/plugins/intake-guard/src/index.ts:71`）。
- **没经过 `submit` 的消息**（调用方直接 `agent.followup`）在循环里补做：intake-guard 用消息自己的上下文、或会话沿用的上下文调用同一个函数，回复相同，只是判定不记在消息上（同文件 :135-142）。单元测试的最后两个用例对比了这两条路径（§2.14）。
...
- **stdout 是排好的一轮。** 脚本模型的回答是 `SCRIPTED-ANSWER`、`[[card:policy_card]]`、`SCRIPTED-CLOSING` 三行，`lyteboat try` 把标记换成了 `[card policy_card]` 这一行（`lyteboat/bundles/try/src/index.ts:102-109`、`:278`）。回答不写标记时，这张 `deferred` 卡跟在回答后面（§3.3 的运行就是这样）；回答写了标记、这一轮却没出卡时，标记被删掉（§6）。
...
- **seq 6 是任务本身。** 它的 `source` 除了 `kind: 'user'`，还带着这次请求的发起者 `lyteboatRequest.owner`（`lyteboat try` 记 `{ kind: 'operator', id: 'cli' }`）和准入判定 `lyteboatRequest.intake`。第三次运行的判定是 `reply`（那里任务是 seq 5），紧接着的 seq 6 就是助手的回复：没有路由、没有 `request/header`、没有模型请求。
...
`tests/policy-desk.spec.ts` 把两个行挂在**单元宿主**上。`createLyteboatUnitHost(adapter)`（`@lyteboat/testing`）按 `CLAUDE.md`「Testing」的 Unit harness 一条搭好宿主：dsh 的不变量注册表、dsh 服务、内核的 agent loop，`adapter` 作为 `mock` provider 的模型，测试结束时自动释放（`lyteboat/tooling/testing/src/index.ts:47-58`）。测试再挂上要测的 lyteboat 服务和本 agent 的两个行。`followUpAndWait(agent, text)` 发一条普通的用户消息并等到 agent 空闲（`index.ts:66-69`），它不经过 `submit`；要走 `lyteboat try` 那条路径，就调 `ctx.intakeGuard.submit` 再 `agent.whenIdle()`。
...
agent 必须有组合测试（`CLAUDE.md`「Testing」的测试表和 Who tests what）。`bootComposition`（`@lyteboat/testing/composition`）在测试进程里按 launcher 的方式启动 `LYTEBOAT_TRY_BUNDLES`（`dsh-base + @lyteboat/host + @lyteboat/business-base + @lyteboat/try`），内部参数放在 `ctx.cmdlineArgs` 上；profile 跳过了要求的 bundle 时启动失败。loader 加载 `lib/`，所以要先构建（`lyteboat/tooling/testing/src/composition.ts:1-21`、`:66`、`:191-205`、`:228-324`）。`createLyteboatScratch(name)` 给每个测试文件一个临时根目录，`run(label)` 在它下面建一对干净的 home 和工作区（`lyteboat/tooling/testing/src/scratch.ts:41-68`）；`scriptedModelEnv(model)` 给出指向脚本模型的环境变量（`scripted-model.ts:173-175`）；`printedSessionId(stderr)` 从 stderr 取会话 id（`composition.ts:126-130`）；`reopenRefusal(records)` 用 dsh 自己的校验器判断日志能不能重开（`lyteboat/tooling/testing/src/session-reopen.ts:19-36`）。
...
- 按**用途**应答，路由请求靠系统文本识别，不要靠调用顺序（`CLAUDE.md`「Testing」的 Unit harness 一条）。`withTitle` 替你回答标题请求（`scripted-model.ts:178-180`）；业务模式不发标题请求，留着它是为了同一个脚本在 `lyteboat web` 那样的组合里也能用。脚本模型只认得 loop、title、router 三种用途（`scripted-model.ts:79-81`）：agent 自己的旁路调用（例如用 `ctx.auxLlm` 做准入分类）到这里也是 `loop`，要靠它自己的系统文本认出来（金融智能体的组合测试用 `准入分类器` 这几个字认，`examples/agents/finance/tests/support/finance-model.ts:32`）。
...
最后一条断言故意不写 `.at(-1)`：它要证明的是这一轮结束了，不是 `turn/end` 恰好是最后一条。业务模式只写一条回退标题，落在 `turn/end` 之前；在带着标题请求的组合里（`lyteboat web`），模型给出的 `session/title` 是异步到达的，可能排在 `turn/end` 之后，`normalizeSessionLog` 为此专门去掉 `session/title*`（`lyteboat/tooling/testing/src/session-log.ts:152`、`:157-169`）。
...
- 准入在 `followup` 之前，由 `submit` 完成（`lyteboat/plugins/intake-guard/src/index.ts:110-125`）。循环里 `lyteboat/intake` 在 pre-assemble 之前派发，reply 时直接返回（`dsh/core/agent-loop/src/agent.ts:279-284`）；intake-guard 的监听在 `next()` 之后读人类消息上的判定（`lyteboat/plugins/intake-guard/src/index.ts:69-75`）。所以准入回复的路径上没有路由调用，也没有 prompt 组装。标题插件在用户消息到达时照常写一个回退标题（上游 `packages/session/session-title/src/index.ts:499-523`）；标题的模型请求来自 `session-title-llm` 行，业务底座关掉了它（`lyteboat/bundles/business-base/cordis.patch.yml`），所以哪条路径上都没有标题请求。§2.13 第三次运行的 seq 8 就是那条回退标题，其 `source.kind` 为 `fallback`。
- tool-policy 在 `next()` **之后**重算限制，skill-router 在 `next()` **之前**路由（`lyteboat/plugins/tool-policy/src/index.ts:121-124`，`lyteboat/plugins/skill-router/src/index.ts:227-230`）。所以同一步里被激活的工具，在本步的请求里就可见。
...
- runtime context 不在 system prompt 里，而是作为 `source.kind: 'runtime-context'` 的 user 消息追加，且只在文本变化时追加（`dsh/core/agent-loop/src/runtime-context.ts:152-163`；它是 `agent/pre-step` 的默认结果，`dsh/core/agent-loop/src/agent.ts:294-300`）。这一步的 user 消息依次是：任务、技能调用消息、dsh-tool-skill 在更外层的 `agent/pre-step` 监听里追加的技能目录（上游 `packages/skill/tool-skill/src/index.ts:213-220`、`:272`）。第一步没有 runtime context：业务底座关掉了 dsh 沙箱与审批的那两段，会话状态又还是空的，文本为空。
- 会话表面推导 `tool/result` 时只取 `data.message`（`dsh/core/session/src/surface.ts:150-152`），所以卡片 payload 和 stateDelta 不会出现在 `tool/result` 消息里。stateDelta 折成 `lyteboatState` 之后，以 `lyteboat:state` runtime context 的形式发给模型（seq 17，§4.8）；卡片 payload 模型永远看不到，模型只写卡片标记。
- stdout 由 `a2ui.turnParts` 从这一轮的日志区间排出来：它按日志顺序读区间里人类消息上的判定卡片、`tool/result` 的 `meta.lyteboat.cards` 和每一步的助手文本，按出卡模式和标记排好（`lyteboat/plugins/a2ui/src/index.ts:151-158`），`lyteboat try` 再把卡片打成 `[card <区域>]` 行（`lyteboat/bundles/try/src/index.ts:102-109`）。它只读日志，不写任何东西。

### 3.2 对应的会话日志片段

日志在 `$LYTEBOAT_HOME/sessions/<编码后的 cwd>/session-<uuid>/session.v4.jsonl.zstd`（带 `--agent` 时 cwd 是 agent 的工作目录 `$LYTEBOAT_HOME/agent-workdirs/<id>`），每次持久写入追加一个 zstd 帧。`@lyteboat/testing/session-log` 的 `readSessionLog` 读文件并逐帧解码（`lyteboat/tooling/testing/src/session-log.ts:97-116`，帧扫描在 `:39`）。第一行是会话头，其中 `"agentPreset":"policy-desk"`。下面是 §2.13 第一次运行的关键记录，已裁剪（去掉 `time`，长字段和 id 用 `…` 代替；`deepseek-flash` 是 dsh-base 配置的默认模型 id）：
...
{"type":"user/message","seq":6,"data":{"content":[{"type":"text","text":"保单 P-1001 还有效吗"}],"source":{"kind":"user","lyteboatRequest":{"owner":{"kind":"operator","id":"cli"},"intake":{"decision":"pass","by":"policy-desk-intake"}}},"role":"user","id":"…"},"surfaceOp":"append"}
...
（这段日志录于 agent 有身份之前。现在的 seq 6 在 `owner` 后面还有 `"agent":{"id":"policy-desk","digest":"sha256:…"}`；`agent.yml` 声明了版本时另有 `version`。）
...
| 3 | `lyteboat/aux-llm-call` | aux-llm，为 skill-router 的路由调用记账（`lyteboat/plugins/aux-llm/src/index.ts:133`；调用方 `skill-router/src/index.ts:363-373`） |
| 5 | `system/message` | persona 段，本步第一次请求前写入；业务底座不加宿主 persona 和 harness 身份段，本例的系统 prompt 就是 persona 那一行 |
| 6 | `user/message`（任务） | `intakeGuard.submit` 用 `requestContext.message(...)` 写成，`source.lyteboatRequest` 带着发起者 `owner` 和准入判定 `intake`（`lyteboat/plugins/intake-guard/src/index.ts:117-123`，`lyteboat/plugins/request-context/src/index.ts:48-53`） |
| 7 | `user/message`（skill-invocation） | skill-router 在 `agent/pre-step` 里追加（`skill-router/src/index.ts:175-182`、`:232-239`） |
...
| 14 | `tool/result` | 内核写入。文本来自 `render`，`meta.lyteboat.cards` 来自工具的 `presentationMeta`（`cardsPresentationMeta`），`meta.lyteboat.stateDelta` 来自 tool-policy 的包装（`tool-policy/src/index.ts:72-92`） |
| 17 | `user/message`（runtime-context） | 只有 `lyteboat:state` 一节，由 `lyteboatState` 投影渲染（`tool-policy/src/index.ts:109-117`，`state.ts:100-103`） |
| 20 | `turn/end` | 内核 |
...
第三次运行（荐股）的人类消息把发起者和整份判定带在身上：`"source":{"kind":"user","lyteboatRequest":{"owner":{"kind":"operator","id":"cli"},"intake":{"decision":"reply","verdict":"out_of_scope","text":"抱歉，我只负责保单查询，不提供股票买卖建议。","by":"policy-desk-intake"}}}`；紧跟着的助手消息 `source` 是 `{"kind":"model","provider":"lyteboat","model":"policy-desk-intake"}`。
...
- **模型自己加载**：dynamic 模式下，业务底座给 agent 留下的 dsh 工具就是 `skill`。路由器没选中时（决策为 `null`、超时或出错），模型可以调用 `skill({ name: 'policy-lookup' })` 读技能正文。投影先记下这次调用要加载的技能，等它的 `tool/result` 成功回来，才把它当成当前技能（`index.ts:144-155`）。下一步是工具之后的续步，没有新的用户输入，不做路由，skill-router 直接激活当前技能的 `requiredTools`（`:298-301`、`:394-397`）；正文已经在那次 `tool/result` 里，不重复追加（`:398-402`）。所以工具从下一步起可见。只在 `mode: dynamic` 下这样处理（`:289-297`）。
...
其他依据：`activate` / `clear` 在 `lyteboat/plugins/tool-policy/src/index.ts:201-221`；路由的粘滞规则在 `lyteboat/plugins/skill-router/src/router.ts:66-87` 和 `src/index.ts:327-377`；「替换而不是累加」在 `src/index.ts:410-422`。「你好」那次运行，路由记录的 `output` 是 `{"skill_id":null,"reason":"寒暄"}`，模型也没加载技能，日志里没有技能调用消息，请求里只有 `skill`（组合测试第二个用例断言了这一点）：
...
- 对账、收尾放在**之后**。tool-policy 等所有人激活完才重算限制（`lyteboat/plugins/tool-policy/src/index.ts:121-124`）。
- 在 `lyteboat/pre-assemble` 里激活工具的写法：`ctx.on('lyteboat/pre-assemble', async (payload, next) => { ctx.toolPolicy.activate(payload.agent, ['x']); return next() })`（参照 `lyteboat/bundles/try/tests/fixtures/plugins/tools.mjs:52-55`；这样的插件要 `inject: ['lyteboatDistro']`，同文件 :9-10）。激活一直有效，直到 `clear(agent)`，或者下一次技能激活把它替换掉（`tool-policy/src/index.ts:201-221`，`skill-router/src/index.ts:410-422`）。路由关闭（`mode: off`）时没有人调 `clear`，一次激活管整个会话。
...
| 类别 | 名字 | 序号 | 出处 |
|---|---|---|---|
| context | `SANDBOX_POLICY` / `APPROVAL_POLICY` / `SUBAGENT_DELEGATION` | 110 / 115 / 120 | `dsh/core/system-prompt/src/index.ts:165-167`；业务模式里写这三段的行（沙箱、审批、子 agent）都被业务底座关掉了 |
| context | `lyteboat:state` | 130 | `LYTEBOAT_STATE_CONTEXT_ORDER`，`lyteboat/core/contracts/src/index.ts:94` |
| section | `HARNESS_IDENTITY` / persona 前缀 | -1000 / 0 | `dsh/core/system-prompt/src/index.ts:126-127`；业务底座把 `system-prompt` 行配成 `includeHarnessIdentity: false`、`personaPrefix: ''`，业务模式里没有 harness 身份段，persona 前缀只来自 agent 自己的 persona 行 |
...
整个 `lyteboatState` 每一步都会作为 `lyteboat:state` 发给模型（`lyteboat/plugins/tool-policy/src/index.ts:109-117`），而每条状态增量又原样留在日志的 `tool/result.meta.lyteboat.stateDelta` 里。所以 delta 要小、不带个人信息，这也是 `policySummary` 只保留六个字段的原因。卡片要用的完整数据直接传给 `renderCard`，不必进状态。
...
- `@lyteboat/tool-policy/agent` 声明了一个没有任何行注册的工具：第一步就失败退出（`lyteboat/plugins/tool-policy/src/index.ts:266-271`）。`activate` 一个未声明的名字，同样抛错（`:201-206`）。这一行或某个工具的策略里写了未知的键（例如去掉了的 `undeclared`、`requiresConfirmation`）：加载时就失败，这个 agent 挂不上（`lyteboat/plugins/tool-policy/src/agent.ts:54-67`）。
...
设计上接受的降级只打 warn：路由调用超时或失败时保持当前技能（原因同时记在那条 `lyteboat/aux-llm-call` 的 `failure` 里，`lyteboat/plugins/aux-llm/src/index.ts:79-91`）、manifest 取值失败（`renderCard` 把警告打成 warn）。这些 warn 在 `lyteboat try` 里默认看不到，§6 讲怎么打开。
...
**续上以后发生什么。** runner 不新建会话，而是先核对日志，再用 `agents.resume` 从日志恢复它（`lyteboat/bundles/try/src/index.ts:190-207`）；dsh 在日志末尾写一条续接标记 `session/end-seed`（`dsh/core/session/src/index.ts:624-625`），新一轮第一次请求的 `request/header` 的 `reason` 是 `resume`。新一轮开始时，各个投影从日志重新折叠：
...
**续聊的前提**（`lyteboat/bundles/try/src/index.ts:172-207`，`lyteboat/bundles/try/src/startup.ts:121-123`），每一条都实测过：
...
**persona 设 `complete: true`（可选）。** prefix 成为完整的系统 prompt，其余段全部不要（上游 `packages/preset/persona/src/index.ts:42-43`、`:67`；组装逻辑在 `dsh/core/system-prompt/src/index.ts:597-633`）：
...
业务模式里系统 prompt 本来就只剩 persona，外加 agent 自己带的 dsh 工具可能附上的使用说明；`complete` 把这些说明和 persona 后缀也去掉，在 `lyteboat web` 里还挡住 dsh 的 harness 身份段和编码工具的说明。金融智能体这样写（`examples/agents/finance/agent.cordis.yml:7-11`）。`lyteboat:state` 这类 runtime context 和路由进来的技能调用消息都是 user 消息，不是段，不受影响；但 `full` 路由模式靠 `lyteboat:skills` **段**（450）列技能，`complete` 会把它一起去掉，所以 `complete` 只和 `dynamic` 模式搭配。
...
- **loop 模型不由 agent 决定。** `@lyteboat/try` 创建或恢复 Agent 时读一次 `agentDefaultModel.currentSelection()`，作为 `agentOptions` 的 provider 和 model 传进去（`lyteboat/bundles/try/src/index.ts:226`、`:256-265`）。preset 定义里没有模型字段（上游 `packages/preset/agent-preset-registry/src/types.ts`、`definition.ts` 里没有 `model`），所以一个进程里所有 agent 用同一个 loop 模型。
- **`agent.yml` 的 `model` 只用来核对。** agent 可以在清单里声明它评测和发布所用的模型（§1.3）。serve 和 eval 打开 agent-catalog 的 `enforceDeclaredModel`（`lyteboat/bundles/serve/cordis.patch.yml:27`、`lyteboat/bundles/eval/cordis.patch.yml:21`）：声明的 provider、model、`reasoningEffort` 和进程的默认模型有一项不同，这个 agent 就不声明，它的代码不会运行，报 `<id>: agent.yml declares model deepseek-official/deepseek-flash, but this process runs deepseek-official/deepseek-pro; run it with that default model (the agent-default-model row) or change agent.yml`（`lyteboat/plugins/agent-catalog/src/index.ts:221-232`）。serve 因此退出 1，eval 退出 2。try 和 `lyteboat web` 不查，所以在 try 里换一个默认模型试 agent 不会被拦住。没声明 `model` 的 agent 哪里都不查。
...
`lyteboat serve` 用 `serve` profile（dsh-base + `@lyteboat/host` + `@lyteboat/business-base` + `@lyteboat/serve`，`lyteboat/apps/cli/src/templates.ts:24-26`）起一个常驻服务，agent 拿到的和 `lyteboat try` 里一样，只有它声明的（§4.11）：`@lyteboat/agent-catalog` 把每个 `--agents` 目录（可重复）里的 agent 都声明成 preset（或者换成 `--release <锁>`，只声明发布过的 agent 并按锁钉住，§4.16），dsh 的 session-controller 不带 Web 界面挂上，`@lyteboat/chat-api` 在 dsh 的 host-webserver 上提供 `POST /chat`、`GET /agents`、`GET /health`（`lyteboat/bundles/serve/cordis.patch.yml:16-65`）。默认监听 `127.0.0.1:8080`、不鉴权；`--host 0.0.0.0` 必须配 `--auth shared-secret`，密钥放在 `--secret-env` 指定的环境变量里（默认 `LYTEBOAT_CHAT_SECRET`），调用方带 `Authorization: Bearer <密钥>`（`lyteboat/bundles/serve/src/startup.ts:82-87`、`:120-124`）。agent 在 `agent.yml` 里声明了模型的，必须和进程的默认模型一致，否则声明失败、服务退出 1（§4.12）。新会话的工作目录是 agent 的工作目录 `$LYTEBOAT_HOME/agent-workdirs/<id>`，和 `lyteboat try --agent`、`lyteboat eval` 的会话在一处，与服务从哪个目录启动无关。有 agent 挂不上时进程退出 1，都挂上了就在 stdout 打一行 `lyteboat serve: http://127.0.0.1:8080/chat (agents: …)`（`lyteboat/bundles/serve/src/index.ts:29-44`）。serve 还挂着运行指标的记录器：每轮结束后往 `$LYTEBOAT_HOME/run-metrics/<UTC 日期>.jsonl` 追加一行（用时、首字时间、步数、工具与技能、结局、发起者），不进模型请求也不进会话日志，Studio 的看板读它（§4.17）。模型的访问方式和 `lyteboat try` 相同（§4.12；不用 key 时照 §2.13 指向脚本模型）。起服务，再发一条消息：
...
`--agents` 可重复、至少一个，`--agent` 必填；`--from` 是运行目录，或 `$LYTEBOAT_HOME/evals` 下的运行 id；`--case <id>` 可重复，只跑这些 id 的用例，用例文件里没有这个 id 就跑不起来（退出 2）；`--run-id <id>` 是运行目录的名字，只能是一段以字母或数字开头、由字母、数字、点、横线、下划线组成的路径，这个目录已经存在就是用法错误（`lyteboat/bundles/eval/src/startup.ts:117-123`、`:139-159`，`lyteboat/plugins/eval-runner/src/index.ts:81-89`）。Studio 的评测界面起运行、试跑单个用例，用的就是这两个参数（§4.17）。每个用例跑完打一行：`✓ <id> (<n> turns)`，或 `✗ <id>: turn <n> <失败的检查>`；最后一行是汇总和报告的路径。全部通过退出 0，有检查失败退出 1，跑不起来（参数不对、用例文件不合法、没有这个 agent、回放缺录音）退出 2（`lyteboat/bundles/eval/src/index.ts:28-47`、`:74-87`）。

**结果在哪。** 每次运行写进 `$LYTEBOAT_HOME/evals/<运行 id>/`，不给 `--run-id` 时，运行 id 是 UTC 开始时间（精确到秒）加 4 位十六进制（`lyteboat/bundles/eval/src/startup.ts:105-108`）。目录里有四样东西（`lyteboat/plugins/eval-runner/src/eval-report.ts:1-8`、`:87-92`）：

- `run.json`：contracts 的 `LyteboatEvalRunRecord`（`lyteboat/core/contracts/src/index.ts:317-351`）。`agent` 是 agent 的身份（id、版本、摘要）；`model` 是录下的循环请求头里的模型，只有 real 运行而且发过循环请求时才有，回放不写，一次运行用了不止一个模型就失败（`lyteboat/plugins/eval-runner/src/index.ts:133-146`）；还有模式、回放的来源 `from`、每个用例是否通过、轮次和检查的总数与通过数、开始时间和耗时。`lyteboat web` 的 Evals 页按这个 schema 读，读不了的（身份出现之前录的运行）跳过并记一条 warn（`lyteboat/plugins/web-pages/src/eval-runs.ts:23-38`）；Studio 的评测界面（§4.17）也按它读，读不了的运行照样列出，只是没有 `run.json` 里的内容，并记一条 warn（`lyteboat/plugins/eval-runner/src/records.ts:1-11`）。
...
**比较两次运行。** `lyteboat eval compare <前> <后>`（两个参数都是运行目录或运行 id）逐个用例、轮次、检查比较两次运行的 `results.jsonl`，列出结果变了的检查：`✗` 是回归，即之前通过、现在失败或没有了；`·` 是其它变化。末行是变化数和回归数，有回归就退出 1（`lyteboat/bundles/eval/src/index.ts:49-58`，`lyteboat/plugins/eval-runner/src/eval-report.ts:110-128`）。改提示要有 eval 数据（`CLAUDE.md`「Agent design」）：改完用新的一次 real 运行和基线比较，看哪些检查变了。

### 4.15 在浏览器里调试 agent：`lyteboat web`

`lyteboat web` 用 `web` profile（dsh-base + `@lyteboat/host` + dsh-web-app + `@lyteboat/web`，`lyteboat/apps/cli/src/templates.ts:21-23`）起 dsh web，再加上 lyteboat 的页面（`@lyteboat/web-pages`）。它读 agent 目录，但不带业务底座：dsh web 自己的编码工具、沙箱、审批、会话标题都在，所以你的 agent 在这里跑的和在 `/chat` 后面跑的不一样（见下面「agent 层不同于业务模式」）。要照 `/chat` 的样子调试 agent，用 `lyteboat eval`（§4.14）或 `lyteboat try --agent`；Studio 工作台（§4.17）按 serve 的样子挂 agent，但只查看、不跑会话。模型的访问方式同 §4.12（不用 key 时照 §2.13 指向脚本模型）。

```sh
node lyteboat/apps/cli/lib/bin.js web --agents ./examples/agents --no-open
```

**参数**（`lyteboat/bundles/web/src/startup.ts:52-101`）：
...
- dsh web 的 `--host`（不许 `0.0.0.0`：浏览器界面能在本机执行代码）、`--port`（`0` 让系统挑一个空闲端口）、`--no-open`、`--trusted-host`（`/api` 的浏览器信任围栏额外接受的主机，可重复）。dsh web 自己的 startup 行遇到不认识的参数（比如 `--agents`）就报错，所以 `@lyteboat/web` 关掉它，由自己的 startup 行照 dsh web 的规则把这些参数再解析一遍，替它发布 `webStartup`（`lyteboat/bundles/web/cordis.patch.yml:1-13`）。

**启动之后**，stdout 先是 dsh web 打的 `dsh web: http://127.0.0.1:<端口>/?token=…`，再是 `lyteboat web: agents <id>, …`；挂不上的 agent 各在 stderr 上打一行 `lyteboat web: agent <id> failed: <原因>`，其余照常服务（`lyteboat/bundles/web/src/index.ts:26-39`）。在浏览器里打开带 token 的地址：它换出 dsh web 的登录 cookie，页面和 lyteboat 的端点都要这个 cookie（`lyteboat/apps/cli/tests/web.e2e.ts:31-40`）。SIGTERM 停下时退出 0。

**agent 目录一改就重新声明。** `@lyteboat/web` 的 agent-catalog 行配 `strict: false`、`watch: true`（`cordis.patch.yml:111-117`）：某个 agent 读不了或挂不上，只在 Agents 页上报告（启动时那一次也打到 stderr），不挡别的 agent；根目录下有文件变了，300 ms 内没有新的变化，就 `reload()`：撤销全部声明，再按目录现在的内容重新声明（`lyteboat/plugins/agent-catalog/src/index.ts:103-104`、`:160-194`）。已经在跑的会话留着它开始时的定义，新会话用新的（[01-architecture.md §2.2](01-architecture.md#22-根-realmstanding-scope-与-agent-scope)）。

**Agents 页和 Evals 页。** dsh web 的左侧栏多出两个入口（`lyteboat/plugins/web-pages/src/client/index.ts:39-47`）：

- Agents：正在服务的 agent（`agent.yml` 的 `name`、`description` 和 id），挂不上的 agent 和原因，一个 Reload 按钮，按一下就是一次 `reload()`（端点 `agents`、`agents/reload`）。
- Evals：`$LYTEBOAT_HOME/evals` 下的运行，新的在前，每行是 agent、通过的用例数和模式；点一行显示它的 `report.md`（端点 `evals`、`evals/report`，`lyteboat/plugins/web-pages/src/eval-runs.ts:23-57`）。它只列 `lyteboat eval`（§4.14）写下的运行，自己不跑评测；运行要写在同一个 `$LYTEBOAT_HOME` 下才看得到。

**会话用哪个 agent。** dsh web 新建会话时用 preset 注册表的默认 preset，`@lyteboat/web` 把它设成 `--agent`（或排第一的那个），在 dsh web 里另挑一个也行（`cordis.patch.yml:9-10`、`:102-105`）。dsh 自带的四个编码 preset（standard、ptc、minimal、cordis）都关掉了，能挑的只有 `--agents` 里的 agent（`:15-25`）。会话头的 `agentPreset` 记着它。

**lyteboat 页签。** 打开一个会话，右侧栏多一个 `lyteboat` 页签（kind `lyteboat`，`lyteboat/plugins/web-pages/src/client/index.ts:49-60`）。上半部分发下一条消息：

- Context 框里写一个 JSON 对象，打开时预填会话当前的请求上下文；写的不是 JSON 对象，页面报错，不发（`SessionTab.tsx:65-114`）。
- 点 Send，页面经 dsh 自己的浏览器传输调端点 `session/send`（`web-pages-client.ts:28-42`）。Host 面用 `ctx.sessionController.prompt`（`queue` 模式）把消息排进会话，请求经 `ctx.requestContext.sourceFields(...)` 记在人类消息上，发起者是 `{ kind: 'operator', id: 'web' }`（`lyteboat/plugins/web-pages/src/index.ts:141-152`）。这是 `/chat` 送消息的那条路（§4.13）：准入在循环第 1 步补做，`reply` 照样作答，判定不记在消息上。空白的消息、多出的字段以 `invalid_request` 拒绝，什么也不排（`index.ts:60-72`）。
...
实测（web 组合测试的夹具 agent `support`，只有一行 persona；脚本模型；按 dsh web 浏览器传输的样子带登录 cookie 调端点）：`session/create` 不指定 agent，会话头的 `agentPreset` 是 `support`；页签发送时带 `{"customer":"c-1"}`，人类消息的 `source` 是 `{"kind":"user","rpcId":"<requestId>","lyteboatRequest":{"requestId":"<同一个 id>","owner":{"kind":"operator","id":"web"},"context":{"customer":"c-1"}}}`（组合测试断言的就是这个，`lyteboat/bundles/web/tests/web.composite.ts:127`）。这里没有 `agent`：`lyteboat web` 的 agent 目录会重新声明，而已经在跑的会话用的是它开始时挂上的代码，此刻目录的摘要说明不了它，所以 `lyteboat web` 不记 agent 的身份（§4.16）。

**agent 层不同于业务模式。** dsh web 把 agent 层（`skill` 工具和技能目录、压缩、文件、shell、子 agent 等工具行）挪进它的编码 preset；`@lyteboat/web` 关掉这些 preset，在宿主上按 dsh-base 的样子把这些行放回来：shell 按平台（非 Windows 上是 `bash`，Windows 上是 `pwsh`），插件管理工具仍然关着（`cordis.patch.yml:27-94`）。少了放回这一步，finance 为 `skill` 写的声明找不到注册它的行，第一步就失败（§4.9 第一条）。`lyteboat web` 不列业务底座，所以放回来的是 dsh-base 的全套 agent 层。实测（Linux，上面那个只有 persona 的 `support`，脚本模型）：循环请求在 `lyteboat web` 下有 24 个工具，在 `lyteboat try` 下只有 `skill`。写了 `inherited: hidden` 的 agent（§2.4）在 `lyteboat web` 里仍只露出自己声明的工具，但系统 prompt、沙箱与审批的 runtime context、会话标题的旁路请求都和业务模式不同。工作区的 AGENTS.md 照 dsh web 的样子不进模型请求，插件包清单也关掉（`cordis.patch.yml:96-100`）。
...
**agent 的身份。** `{ id, version?, digest }`（`LyteboatAgentIdentity`，`lyteboat/core/contracts/src/index.ts:298-315`）：`id` 是目录名，`version` 是 `agent.yml` 声明的，`digest` 是目录内容的摘要，agent-catalog 每次声明 agent 时算（`lyteboat/plugins/agent-catalog/src/agent-digest.ts:1-12`）：
...
try（经 `intakeGuard.submit` 的 `agent`）、`/chat`、eval 把身份写进每条人类消息的 `source.lyteboatRequest.agent`（`lyteboat/bundles/try/src/index.ts:271`、`lyteboat/plugins/chat-api/src/index.ts:273`、`lyteboat/plugins/eval-runner/src/index.ts:192`）；eval 还写进 `run.json`（§4.14），`GET /agents` 列出版本（§4.13）。`lyteboat web` 不写（§4.15）。
...
`lyteboat release` 就是 `lyteboat eval release`：同一个 eval profile，跑 eval app 的 `release` 子命令，也接受 `--profile`、`--patch`、`--plugin`（`lyteboat/apps/cli/src/args.ts:159-169`，`lyteboat/bundles/eval/src/startup.ts:175-186`）。eval profile 打开了 `enforceDeclaredModel`，所以发布时 `agent.yml` 的模型也必须是进程的默认模型。

**闸门的六步**（`evalRunner.release()`，`lyteboat/plugins/eval-runner/src/index.ts:159-164`，逐步在 `eval-release.ts:130-154`），按顺序，哪一步不过就停在哪一步：
...
**锁里有什么。** `LyteboatAgentRelease`（`lyteboat/core/contracts/src/index.ts:353-383`）：`agent`（id、版本、摘要）、`model`、`dshBase`（这个构建的内核来自哪个 dsh 版本）、`files`（摘要背后的逐文件哈希）、`baseline`（`startedAt`，用例、轮次、检查的个数，`results.jsonl` 的 sha256）。键的顺序固定、两空格缩进、结尾一个换行，所以同样的发布写出同样的字节，锁的 diff 可以像代码一样审。逐字段的表见 [02-distribution.md](02-distribution.md) §9.3。

**运维怎么用：`lyteboat serve --release <agent>/agent.release.json`。** `--release` 可重复，和 `--agents` 互斥。锁所在的目录就是 agent 目录，目录名必须等于锁里的 id；锁的 `dshBase` 必须等于这个构建的 dsh 版本，在 `/chat` 的任何一行启动之前就查；agent-catalog 只声明锁里的 agent，并且在它的代码运行之前比版本、比摘要，摘要不同时列出变了、多了、少了的文件；serve 的 `enforceDeclaredModel` 再要求进程的默认模型等于 `agent.yml` 声明的（也就是锁里的）模型。任何一项不对，服务退出 1，`/chat` 不开。每一项的原话和实测见 [02-distribution.md](02-distribution.md) §9.3。Studio 的雷达也读 agent 旁边的锁：目录的摘要和锁里的不同，就把这个 agent 标为偏离发布，`serve --release` 这时会拒绝它（§4.17）。

**锁管住什么、管不住什么。** 这是运维规则，[02-distribution.md](02-distribution.md) §9.3 有同样的两张表：

- 锁管住：agent 目录的内容（摘要和逐文件列表）、版本、模型（serve 核对进程的默认模型等于它）、内核的 dsh 版本（`dshBase`）、基线证据。
- 锁管不住 lyteboat 框架代码（`lyteboat/` 下的插件和 bundle）：用跑 `lyteboat release` 的同一个 lyteboat 构建去 serve。
...
`lyteboat studio` 用 `studio` profile（dsh-base + `@lyteboat/host` + `@lyteboat/business-base` + `@lyteboat/studio`，`lyteboat/apps/cli/src/templates.ts:30-32`）起 Studio 工作台：原 Studio 的页面，数据来自 lyteboat。它和 serve 一样带业务底座，agent 的工具、技能和路由按 serve 的样子挂上，所以这里看到的工具面、技能和诊断，就是 `/chat` 后面那一套（§4.11）。它**不跑会话**：不挂 session-controller，不建也不续会话；会话页、看板和评测界面读的是 serve 和 `lyteboat eval` 在同一个 `$LYTEBOAT_HOME` 里留下的东西（[01-architecture.md §1.5](01-architecture.md#15-studio-工作台另一个进程同一个-lyteboat_home)）。要让 agent 答一轮，仍然用 `lyteboat try --agent`、`/chat` 或 `lyteboat eval`。

**先建账户，再起 Studio。** Studio 用自己的账户登录；一个账户都没有时它不启动，在 stderr 上给出建账户的命令，退出 1。口令只从标准输入读一行，不能是终端，也不能是空行，所以它不会留在 shell 历史和进程列表里（`lyteboat/bundles/studio/src/startup.ts:162-168`）：
...
- `account add <用户名> --role <admin|editor|viewer>` 建账户并授予角色；`--display-name` 是 Studio 显示的名字，`--user-id` 是角色和审计日志记的 id，默认都是用户名。用户名已存在就报错，改口令用 `account set-password <用户名>`；`account remove <用户名>` 删账户，它的角色留着，直到 admin 在 Users 页撤销；`account list` 列出账户和角色（`startup.ts:176-207`）。
...
挂不上的 agent 各在 stderr 上打一行 `lyteboat studio: agent <id> failed: <原因>`，其余照常服务（`lyteboat/bundles/studio/src/index.ts:33-47`）。在浏览器里打开这个地址，用账户登录；`/` 也跳到这里。进程不会自己退出，SIGTERM 停下时退出 0。
...
**读会话。** 会话页列的是 agent 工作目录 `$LYTEBOAT_HOME/agent-workdirs/<id>` 里的会话，也就是 `/chat` 的（发起者 `user:<user_id>`）和 `lyteboat try --agent` 的（`operator:cli`）；评测的会话（`system:eval`）和请求指明别的 agent 的会话不列（`lyteboat/plugins/session-index/src/index.ts:157-160`）。
...
看板的数字按原 Studio 的算法算（最近秩百分位、按桶统计、同时有一轮在跑的用户峰值），用户就是请求的发起者，准入作答的轮算受控退出（`lyteboat/plugins/studio-api/src/studio-dashboard-health.ts:1-11`）。
...
Studio 起的每个运行，都是启动器 bin 的一个 `lyteboat eval` 子进程：`--agents <Studio 的每个目录> --agent <id> --run-id <新 id>`，回放加 `--model replay --from <那次运行的目录>`，试跑加 `--case <id>`（`lyteboat/plugins/studio-api/src/studio-eval-jobs.ts:129-162`）。它和你在命令行跑的一样写进 `$LYTEBOAT_HOME/evals/<运行 id>/`，输出记在 `$LYTEBOAT_HOME/studio/eval-jobs/<运行 id>.log`，进度按输出里的 ✓ / ✗ 行数。所以：
...
**热修技能，以及它对已发布 agent 意味着什么。** admin 在技能页打开编辑器，改整份 SKILL.md 保存（`PUT /api/studio/agents/<id>/skills/<名字>`，`lyteboat/plugins/studio-api/src/studio-workspace-routes.ts:76-99`）：

- 保存带着读到时文件的 sha256（`If-Match`）；这期间文件被改过就回 412，重新读了再改。
- 新文本要过技能加载器和路由器的规则：保留 `---` 包着的 YAML frontmatter、同一个 `name`、一个 `description`、能解析的 `metadata.lyteboat`、`requiredTools` 里只有 agent 注册并声明了的工具（§4.3）；不合就回 400，说出违反的是哪一条（`studio-skill-hotfix.ts:1-10`）。
...
热修改的就是 agent 目录本身，它是 agent 的一次改动，要像别的改动一样提交或丢弃。对一个有发布锁的 agent，Studio 保存前先弹出警告：`这个 agent 有发布锁（v<版本>）。保存后目录内容与锁不同：雷达显示「偏离发布」，按锁上线的 serve 重启时会拒绝它。技能正文在 serve 里下次调用时生效，名称和描述要等 serve 重启。这次修改会写进审计记录。`（`lyteboat/plugins/studio-web/src/client/studio-agent-skills.tsx:51-53`）。也就是说：
...
| `error: agent "policy_desk" not found in the --agents directories (available: finance, policy-desk)`，退出 1（实测） | id 写错了（id 就是目录名），或者目录里没有 `agent.cordis.yml` | 用列出来的 id；确认文件存在（`lyteboat/bundles/try/src/startup.ts:116-118`） |
...
| 准入没起作用，荐股问题照样到了路由器和模型 | 看这条人类消息的 `source.lyteboatRequest.intake`：没有这个字段，说明这个 agent 的作用域链上没有登记准入函数，`submit` 没有判定可记（`lyteboat/plugins/intake-guard/src/index.ts:127-133`）；`decision` 是 `pass`，说明函数判了放行 | 确认准入行在 `agent.cordis.yml` 里、`lib/` 已构建、`inject` 里有 `intakeGuard`；再查 `admit` 的判断，单元测试覆盖它（§2.14） |
...
| `error: --context file not found: <工作目录>/nosuch.json`、`error: --context is not JSON: Unexpected end of JSON input`（均实测）或 `error: --context must be a JSON object` | `--context` 的值不以 `{` 开头时被当成文件路径（`[1]` 也会被当成文件名）；内联的 JSON 写坏了；或者文件里解析出来不是对象 | 内联写 `'{"channel":"app"}'`，或者给一个装着 JSON 对象的文件（`lyteboat/bundles/try/src/startup.ts:61-74`） |
...
| 成员 | 说明 |
|---|---|
| `register(definition, meta = {})` | 在调用方作用域的层里注册工具并登记 meta（注册过的工具就算声明过）；带 `stateDelta` 时包装 `presentationMeta`；返回 disposer（:134-148） |
| `declare(name, meta)` | 给别的行注册的工具登记 meta，近的作用域优先；返回 disposer（:157-163） |
| `declareInherited(visibility)` | 在调用方作用域声明「没有任何声明点名的继承工具」是否可见：`visible` 或 `hidden`，近的作用域优先，没有声明时为 `visible`；一个作用域只能声明一次；`hidden` 隐藏的工具无法激活；返回 disposer（:174-184）。agent 行的 `inherited` 配置就是调用它 |
| `metaOf(name, agent?)` | 某个 agent 解析到的 meta（:191-193） |
| `activate(agent, names)` | 让 `auto` 工具对该 agent 可见，名字必须已声明，否则抛错；一直有效，直到 `clear`（:201-210） |
| `clear(agent)` | 撤销该 agent 的所有激活（:216-221） |
| `activated(agent)` / `visible(agent)` | 已激活的名字（排序后）/ 模型当前看到的工具（:224-231） |

`LyteboatToolMeta`（`lyteboat/core/contracts/src/index.ts:106-115`）：`visibility?: 'always' | 'auto'`（默认 `always`）、`stateDelta?: (args, value) => JsonValue | undefined`。`stateDelta` 必须返回以点路径为键的 JSON 对象或 `undefined`；非对象或有空段的路径会让投影折叠抛错、运行失败（`lyteboat/plugins/tool-policy/src/state.ts:53-62`，§2.10）。agent 行里声明的策略是去掉 `stateDelta` 的 `LyteboatToolPolicy`（`lyteboat/plugins/tool-policy/src/index.ts:45`）。
...
| `turnParts(session, fromSeq)` | 把从 `fromSeq` 开始的一轮排成 `LyteboatTurnPart[]`（`{ kind: 'text', text }` 或 `{ kind: 'card', card }`）：每一步的回答文字按写出的顺序保留，immediate 卡放在它的结果到达的位置，回答里的 `[[card:<区域>]]` 换成这个区域的延迟卡，本轮正常结束时没放下的 `deferred` 卡跟在后面、`deferred_discard` 卡丢掉；只读日志，不写（:143-158，`turn-parts.ts:67-177`）。`lyteboat try` 的 stdout 就是它的结果 |
...
一个工具结果的卡片放在 `meta.lyteboat.cards`，每张是 `LyteboatResultCard`，`emission` 为 `'immediate' | 'deferred' | 'deferred_discard'`（`lyteboat/core/contracts/src/index.ts:133-142`）；准入判定的卡片形状相同，放在人类消息的 `source.lyteboatRequest.intake.cards`。两处都按 contracts 的 zod schema 读，信封不合 schema 时投影抛错（`cards-projection.ts:52-65`）。
...
### 7.6 投影（`lyteboat/core/contracts/src/index.ts:566-587`）
...
`lyteboatState`、`lyteboatCards`、`lyteboatActiveSkill` 的 `stateVersion` 是 1，`lyteboatRequest` 是 3。在 agent 行里直接用 `ctx.sessionProjections` 要补齐四处。宿主插件 `@lyteboat/request-context` 用的是同一套写法，下面引它的行号。四处都在一份 policy-desk 的拷贝上实测过：
...
|---|---|---|
| `requestContext` | `@lyteboat/request-context`，`lyteboat/plugins/request-context` | `message(text, request)`：把一条人类消息连同请求（`requestId?`、`owner?`、`agent?`、`traceId?`、`context?`、`intake?`）写成 user 消息，什么都不带时 `source` 就是 `{ kind: 'user' }`（:48-53）；`requestOf(message)`：读回消息带的请求，信封不合 schema 时抛错（:71-73）；`contextOf(agent)`：会话当前的请求上下文，没有时为 `{}`（:76-78） |
| `intakeGuard` | `@lyteboat/intake-guard`，`lyteboat/plugins/intake-guard` | `register({ name, admit })`：在调用方作用域登记准入函数，近的作用域优先，返回 disposer（:83-89）；`admissionFor(agent)`：这个 agent 用哪个准入函数（:92-98）；`submit(agent, { text, context?, requestId?, owner?, agent? }, signal)`：准入，再把请求写成人类消息 `followup` 给 agent，返回记下的判定，没有准入函数时为 `undefined`；空的 `context` 和不带一样，沿用会话的上下文；准入期间 `signal` 中止时什么也不交给 agent（:110-133） |
