...
/** Start the built boat launcher without waiting for it to exit. */
export function startBoat(args: readonly string[], options: BoatSpawnOptions = {}): BoatChild {
  const child = spawn(process.execPath, [BOAT_BIN, ...args], {
...
/** Run the built boat launcher to completion. */
export async function runBoat(args: readonly string[], options: BoatSpawnOptions = {}): Promise<BoatRunResult> {
  const handle = startBoat(args, options)
