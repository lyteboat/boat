...
lyteboat/                 lyteboat's own packages, one directory per layer
...
| `lyteboat/core/contracts` | `@lyteboat/contracts` | lyteboat's declarations over the dsh seams: tool and skill metadata, the kernel's `lyteboat/*` events (re-exported), log nodes, `LyteboatDistro`, and the zod schemas of the JSON types it declares |
...
- Tracks dsh **0.1.7-rc.2** (`dsh.upstream.json`). The kernel is its import plus lyteboat's three registered extensions (`lyteboat/intake`, `lyteboat/pre-assemble`, `session-append-ignorable`), and every gate above passes against it.
