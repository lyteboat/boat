...
- Known limitations:
  - A message sent from dsh's own composer in Studio carries no request context; the lyteboat tab sends one that does. dsh's conversation shows a card as its marker only; the tab shows the cards as JSON.
