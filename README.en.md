...
| `lyteboat eval [options]` | Runs an agent's eval cases and checks every turn (profile `eval`); `--case <id>` runs only those cases, `--run-id <id>` names the run's directory (which must not exist); `lyteboat eval compare <before> <after>` compares two runs |
...
| `lyteboat/bundles/studio` | `@lyteboat/studio` | The bundle behind `lyteboat studio`: it lists the business base, so an agent's tools and skills mount as serve mounts them; it declares every agent of the `--agents` directories (reloaded when a directory changes, a failure reported on the radar) and mounts agent-inspector, session-index, the run-metrics reader, the eval records (`@lyteboat/eval-runner/records`), studio-auth, and studio-api; an eval run the Studio starts is a `lyteboat eval` process; no session controller, so Studio never creates or continues a session; its `account` commands manage the accounts |
...
| `lyteboat/plugins/agent-inspector` | `@lyteboat/agent-inspector` | What an agent is made of, read from its standing scope without creating an agent instance or writing to disk: its tools (their declaration, how each reaches the model: always, once activated, or never, and the skills that require it), its skills (required tools, metadata problems, the SKILL.md path), how its skills are routed, and each skill's deterministic checks (rule ids and the tools a rule failed on; the pages word them) |
| `lyteboat/plugins/session-index` | `@lyteboat/session-index` | An agent's stored sessions, read with read handles that never take a session's write ownership: listed by the agent's working directory (newest first, by time window and owner, a page at a time; eval runs and sessions whose requests name another agent left out), searched by session id, human message, or trace id among the 500 newest of the window, one session folded into its timeline (request and admission, activated skills, tool calls and results, card areas, state deltas, side model calls, model timing and tokens, compaction and pruning, imported history), or one read as stored |
| `lyteboat/plugins/run-metrics` | `@lyteboat/run-metrics` | Run metrics: the recorder (mounted by serve) appends one line per turn after it ends to `$LYTEBOAT_HOME/run-metrics/<day>.jsonl` (duration, time to first content, steps, tools and skills, outcome, owner) and keeps a heartbeat file per process of the turns it runs; the files are written through an asynchronous queue, a failed write is logged and never reaches the turn, and nothing enters a model request or the session log; day files past the retention are removed at start. The reader (`./reader`, mounted by the Studio) answers the turns of a time range and the running turns of heartbeats from the last 30 seconds |
...
| `lyteboat/plugins/agent-def` | `@lyteboat/agent-def` | A business agent's definition: `lyteboatAgentDef({…})` checks one declaration (`agentId`, `agentName`, persona, skill directories, skill routing, tool policy, model request overrides, tools, admission, the `render_a2ui` tool, event listeners) and returns the agent directory's one row, which hands each declaration to the host service that owns it when it mounts |
...
| `lyteboat/plugins/studio-web` | `@lyteboat/studio-web` | Studio's pages: a React single-page app ported from the original Studio (`src/client`, its layout and styles kept, its data now from `/api/studio`; an agent's workspace has Overview, Skills with deterministic diagnostics and an admin's hot-fix of a SKILL.md, Tools, and Sessions (the list, search, a timeline, the stored form); the Dashboard has the performance view, by agent and time range with a date comparison, and the static view: totals, coverage, running messages, activity; Evals, in a window of their own: the case files, read only, with a trial run of one case for an editor or admin; the runs, filtered, deleted in bulk, and two picked to compare; a new run, real or a replay of a real run; one run's cases turn by turn with their checks, stopped, rerun, exported as JSON, or deleted; and two runs compared), built by Vite into `lib/web` in `pnpm run build`; the host face serves them at `/studio` (a page path answers `index.html`, a missing file under `/studio/assets/` is 404), every answer under a CSP that admits only same-origin scripts and styles, and `/` redirects to `/studio/` |
| `lyteboat/plugins/studio-api` | `@lyteboat/studio-api` | Studio's HTTP API at `/api/studio` on the host web server: a Host allowlist (against DNS rebinding), `nosniff` and a CSP on every answer, role checks, request bodies checked strictly against the contracts' schemas; sign-in, users and roles, the System page (secrets in the environment masked), the trace link, the agent radar (version, digest, release lock, whether the agent deviates from it), and an agent's workspace (skills and routing, one skill's body and file, deterministic diagnostics, tools, the session list and search, a session's timeline and stored form), and the Dashboard (the original Studio's figures, checked against its golden fixtures: the performance view by bucket with a comparison window and the tool and skill rankings, the static view, the running turns), and Evals (the case files read only, the runs and one run's cases, two runs compared; an editor or admin starts a real or replay run, stops it, and deletes one: a `lyteboat eval` process, one per agent and two overall, `interrupted` when the Studio restarts while it runs); an admin's hot-fix of an existing skill's SKILL.md (with `If-Match`, checked against the skill loader's rules first, followed by a reload, and restored when the loader refuses it); every change is appended to an audit log |
