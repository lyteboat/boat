/**
 * Boot a composition in the test process, the way the `boat` launcher boots a
...
export interface CompositionOptions {
  /** Bundle packages in layer order, e.g. `['@deepseek-ai/dsh-base', '@boat/host', '@boat/run']`. */
  bundles: readonly string[]
  /** Layers above the bundles: row overrides and inserted rows (see {@link pluginFileRow}). */
  patches?: readonly PatchOptions[]
  /** The inner arguments, as they would follow `boat run` on a command line. */
...
export async function bootComposition(options: CompositionOptions): Promise<CompositionRun> {
  const home = options.home ?? mkdtempSync(join(tmpdir(), 'boat-composition-'))
