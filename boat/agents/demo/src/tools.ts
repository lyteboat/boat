...
function personaName(): string {
  return process.env['BOAT_DEMO_PERSONA'] ?? 'healthy'
}
...
export async function apply(ctx: Context): Promise<void> {
  ...
  ctx.toolPolicy.register(defineTool({
    ...
    output: {
      ...
      presentationMeta: (_args, value) => ({
        boat: { card: { surfaceId: String((value.card as Record<string, unknown>)['surfaceId'] ?? ''), payload: value.card } },
      }),
    },
  ...
  ctx.toolPolicy.register(defineTool({
    ...
    execute: async (_args, exec) => {
      ...
      const state = agent === undefined ? undefined : ctx.sessionProjections.stateOf(agent.session, 'boatState')
