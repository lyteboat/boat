...
import { authState, buildAssetsBundle, diagnose, loadPersona } from '@boat/agent-demo/assets'
...
describe('buildAssetsBundle', () => {
  it('folds the healthy persona into a fully authorized bundle with bucket shares', () => {
    const { yl_assets, yl_assets_raw } = buildAssetsBundle(loadPersona(PERSONAS, 'healthy'))
    expect(yl_assets).toMatchObject({
    ...
    const buckets = yl_assets['buckets'] as Record<string, { pct: number; amount: string; authorized: boolean }>
    ...
    expect(yl_assets_raw.accounts).toHaveLength(3)
  })

  it('leaves unauthorized areas out of the totals and counts the missing main groups', () => {
    const { yl_assets, yl_assets_raw } = buildAssetsBundle(loadPersona(PERSONAS, 'partial'))
    expect(yl_assets).toMatchObject({ total: '42000.00', auth_state: 'multi', insurance_authorized: false, unauth_main_group_count: 2, policy_count: 0 })
    const buckets = yl_assets['buckets'] as Record<string, { pct: number; authorized: boolean }>
    expect(buckets['进取']).toMatchObject({ pct: 0, authorized: false })
    expect(yl_assets_raw.accounts.map(account => account.name)).toEqual(['活期', '理财'])
  })
...
describe('diagnose', () => {
  it('judges each bucket against its band and marks unauthorized buckets', () => {
    const healthy = diagnose(buildAssetsBundle(loadPersona(PERSONAS, 'healthy')).yl_assets)
    expect(healthy.map(verdict => verdict.judge)).toEqual(['ok', 'ok', 'ok'])
    const partial = diagnose(buildAssetsBundle(loadPersona(PERSONAS, 'partial')).yl_assets)
