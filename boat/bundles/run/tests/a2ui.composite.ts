...
describe('@boat/a2ui in the run composition (in process, scripted model)', () => {
  ...
  it('renders a card from the state a tool folded in, puts it on tool/result.meta, and shows only the digest to the model', async () => {
    ...
    try {
      ...
      expect(queried?.data?.['meta']).toMatchObject({ boat: { stateDelta: { profile: { name: 'Composite Tester' } } } })
      ...
      expect(meta.boat.card.surfaceId).toMatch(/^summary-/u)
      expect(meta.boat.card.payload['rootComponentId']).toBe('root')
      expect(JSON.stringify(meta.boat.card.payload)).toContain('Composite Tester')
    } finally {
      await model.close()
    }
  })
