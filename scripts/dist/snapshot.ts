/**
 * Write `dsh-compat/contract/dsh-<version>/` for a dsh checkout at a tag: the contract
...
function printDifference(from: string, to: string): void {
  ...
  const persistence = (version: string): Map<string, string> => {
    const { roots } = JSON.parse(readFileSync(join(repoRoot, 'dsh-compat/contract', `dsh-${version}`, 'persistence.json'), 'utf8')) as { roots: unknown }
...
async function main(): Promise<void> {
  ...
  const out = join(repoRoot, 'dsh-compat/contract', `dsh-${release.dsh}`)
  ...
  if (pinned !== release.dsh && existsSync(join(repoRoot, 'dsh-compat/contract', `dsh-${pinned}`))) printDifference(pinned, release.dsh)
}
