 ...
 * is compared key by key with `contract/dsh-<version>/`:
 *
 ...
 *   entry of `contract/extensions.yml` lists it (or a prefix of it);
...
/** One entry of `contract/extensions.yml`. */
...
export function readExtensions(): ContractExtension[] {
  const parsed = parse(readFileSync(join(repoRoot, 'contract/extensions.yml'), 'utf8')) as { extensions?: ContractExtension[] | null }
  const extensions = parsed.extensions ?? []
  const ids = new Set<string>()
  for (const extension of extensions) {
    for (const field of ['id', 'package', 'kind', 'surface', 'since', 'exit'] as const) {
      if (typeof extension[field] !== 'string' || extension[field] === '') throw new Error(`contract/extensions.yml: an entry lacks "${field}"`)
    }
    if (!Array.isArray(extension.contract) || !Array.isArray(extension.tests)) throw new Error(`contract/extensions.yml: ${extension.id} needs "contract" and "tests" lists`)
    if (ids.has(extension.id)) throw new Error(`contract/extensions.yml: duplicate id ${extension.id}`)
...
export function readSnapshot(version: string): Map<string, string> {
  ...
  for (const file of CONTRACT_FILES) {
    flatten(JSON.parse(readFileSync(join(repoRoot, 'contract', `dsh-${version}`, `${file}.json`), 'utf8')), [file], out)
  }
  return out
}
