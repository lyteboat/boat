 ...
 * - an `extend` commit names its `contract/extensions.yml` entry in
...
function violations(commits: readonly DistCommit[]): string[] {
  ...
  for (const commit of commits) {
    ...
    for (const id of commit.trailers.get('Dist-Extension') ?? []) {
      if (!extensionIds.has(id)) found.push(`${where}: Dist-Extension ${id} is not in contract/extensions.yml`)
      named.add(id)
    }
  }
  for (const id of extensionIds) if (!named.has(id)) found.push(`contract/extensions.yml: ${id} is named by no Dist-Extension commit`)
  return found
}
