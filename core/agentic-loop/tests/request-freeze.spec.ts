...
import { mountAgentLoopTestDependencies } from '@boat/dsh-agent-loop-testkit-fork'
...
// boat adaptation: upstream spies on the `deepFreeze` export with vi.spyOn, which
// vitest cannot do on a real ESM namespace (the published package). The module is
// replaced with a copy whose deepFreeze is a vi.fn wrapping the original.
const realDeepFreeze = vi.hoisted(() => ({ current: undefined as (<T>(value: T) => T) | undefined }))
vi.mock('@deepseek-ai/dsh-util-values', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@deepseek-ai/dsh-util-values')>()
  realDeepFreeze.current = actual.deepFreeze
  return { ...actual, deepFreeze: vi.fn(actual.deepFreeze) }
})
...
afterEach(async () => {
  try {
    ...
    cleanups.length = 0
    vi.restoreAllMocks()
    vi.mocked(values.deepFreeze).mockReset()
  }
})
...
describe('loop-owned request freezing', () => {
  it('adopts restored identities, freezes nested messages at dispatch, and leaves event wrappers mutable', async () => {
    ...
    const freeze = vi.mocked(values.deepFreeze)
    freeze.mockClear()
  ...
  it('retries freezing an identity whose previous traversal failed', async () => {
    ...
    const realFreeze = realDeepFreeze.current
    if (realFreeze === undefined) throw new Error('deepFreeze original not captured')
    let traversals = 0
    vi.mocked(values.deepFreeze).mockImplementation(((value: unknown) => {
      if (value === message && ++traversals === 1) throw new Error('freeze traversal failed')
      return realFreeze(value)
    }) as typeof values.deepFreeze)
    const errors: unknown[] = []
    const requests: GenerateOptions[] = []
    ctx.on('agent/error', ({ error }) => { errors.push(error) })
