 ...
 * with boat's kernel write the same session log, event for event after
...
describe('G4: the official release and boat write the same session log', () => {
  let vanilla: string
  let boat: string
  let root: string
  ...
  it.each(G4_SCENARIOS.map(scenario => [scenario.name, scenario] as const))('%s', async (_name, scenario) => {
    ...
    const ours = await runScenario(boat, scenario, freshRun(root, `${scenario.name}-boat`, scenario.files))
