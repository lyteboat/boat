/**
 * Adapted from deepseek-ai/deepseek-harness @ dsh-v0.1.7-alpha.2 (00102833),
 * scripts/test-invariants.ts, MIT. boat changes: companions are globbed from
 ...
 * name non-kernel files are empty; nothing else.
 */
/**
...
export function usesManualInvariantTree(testPath: string): boolean {
  ...
  if (/\/dsh\/[^/]+\/[^/]+\/tests\/[^/]*invariant[^/]*\.spec\.ts$/.test(normalized)) return true
  return MANUAL_INVARIANT_TEST_EXCEPTIONS.some(path => normalized.endsWith(path))
}

const ALL_COMPANION_TESTS: readonly string[] = []
