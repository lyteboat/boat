...
- D0 — distribution tooling (`scripts/dist`): the contract snapshot of the tracked dsh release (`contract/`), the per-tag import, the delta report, the overlay gates.
...
- D2 — conformance against the official release: G4 (same session log for the same scripted run), G5 (23 pinned community plugins installed with `dsh plugin add`), G6 (sessions cross both ways) in `pnpm run conformance`; G3 and the persistence gate in the sync pipeline.
