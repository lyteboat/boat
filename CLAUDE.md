...
lyteboat/
  ...
  bundles/business-base/  @lyteboat/business-base — the business modes' base (cordis.patch.yml only; try, serve, eval, and studio list it after host): the coding tools, sandbox, approval, and the rows that bring the machine into model requests off
  ...
  bundles/studio/         @lyteboat/studio — the bundle behind `lyteboat studio`, the Studio workshop (cordis.patch.yml, startup flags, the `account` commands): the agent catalog over the business base (reloaded when a root changes), the host web server with studio-auth and studio-api; no session controller
  ...
  plugins/studio-api/     @lyteboat/studio-api — Studio's HTTP API at /api/studio on the host web server: Host allowlist, security headers, role checks, sign-in, users, the System page, the agent radar, the audit log
...
| Serve the Studio workshop | `node lyteboat/apps/cli/lib/bin.js studio --agents ./examples/agents` (`/api/studio`; `--port`, `--host`, `--trusted-host`, `--gateway-secret-env`) |
