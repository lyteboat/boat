...
- **Functions** are a verb and its object and say what they do or return: `readAgentManifest`, `declareInheritedTools`. A lookup reads `<thing>Of<source>` (`customerOfContext`); a predicate reads as a yes/no question (`isAgentDirectory`, `hasStateDelta`). `handle`, `process`, `run`, `do`, `manage` alone name no action.
