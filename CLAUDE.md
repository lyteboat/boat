...
- Skill names are hyphenated (`asset-overview`), matching `/^[a-z0-9]+(?:-[a-z0-9]+)*$/`; the reference implementation's underscore ids are renamed on migration. lyteboat's skill metadata is the `metadata.lyteboat` object of the SKILL.md frontmatter (`requiredTools`).
