...
- **Fixtures** belong to the package whose tests use them: agent directories and plugin files for the headless composition under `lyteboat/bundles/headless/tests/fixtures`, `--plugin` files for the launcher under `lyteboat/apps/cli/tests/fixtures/plugins`, templates under `lyteboat/plugins/a2ui/tests/fixtures`, reference baselines under `tests/fixtures/baseline`. Fix the fixture, never the normalizer.
