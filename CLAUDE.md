...
- **Fixtures** belong to the package whose tests use them: agent directories and plugin files for the headless composition under `lyteboat/bundles/headless/tests/fixtures`, the serve composition's agents under `lyteboat/bundles/serve/tests/fixtures`, the finance agent's `/chat` frame goldens under `examples/agents/finance/tests/fixtures/frames` (masked clock, session, and surface ids; `vitest -u` rewrites them, and a rewrite is reviewed like code), `--plugin` files and a serve agent for the launcher under `lyteboat/apps/cli/tests/fixtures`, templates under `lyteboat/plugins/a2ui/tests/fixtures`, reference baselines under `tests/fixtures/baseline`. Fix the fixture, never the normalizer.
