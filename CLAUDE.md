...
lyteboat/
  ...
  plugins/agent-inspector/ @lyteboat/agent-inspector — what an agent is made of, read from its standing scope without an agent instance: its tools and how each reaches the model, its skills, its routing (ctx.agentInspector)
  ...
  plugins/studio-api/     @lyteboat/studio-api — Studio's HTTP API at /api/studio on the host web server: Host allowlist, security headers, role checks, sign-in, users, the System page, the agent radar, an agent's workspace and sessions, the Dashboard, the admin's skill hot-fix, the audit log
