...
lyteboat/
  ...
  plugins/studio-api/     @lyteboat/studio-api — Studio's HTTP API at /api/studio on the host web server: Host allowlist, security headers, role checks, sign-in, users, the System page, the agent radar, an agent's workspace and sessions, the admin's skill hot-fix, the audit log
