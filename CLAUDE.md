...
lyteboat/
  ...
  bundles/studio/         @lyteboat/studio — the bundle behind `lyteboat studio`, the Studio workshop (cordis.patch.yml, startup flags, the `account` commands): the agent catalog over the business base (reloaded when a root changes), the host web server with studio-auth, studio-api, and studio-web; no session controller
