...
describe('lyteboat run --plugin <intake gate file> (built bin, mock model)', () => {
  let root: string

  beforeAll(() => {
    root = mkdtempSync(join(tmpdir(), 'lyteboat-intake-'))
  ...
  async function run(label: string, task: string) {
    ...
    let result
    try {
      result = await runLyteboat(
        ['run', '--plugin', PLUGIN, '--patch', join(root, 'disable-title-llm.patch.yml'), task],
        { cwd: workspace, env: { LYTEBOAT_HOME: home, DEEPSEEK_BASE_URL: `${mock.baseURL}/v1`, DEEPSEEK_API_KEY: 'mock-key', DSH_TELEMETRY_DISABLED: '1' } },
      )
    } finally {
      await mock.close()
    }
  ...
  it('answers an out-of-scope task with the fixed reply and zero model requests', async () => {
    const { result, requests, types } = await run('gated', '帮我炒股')
  ...
  it('lets an in-scope task through to the model', async () => {
    const { result, requests } = await run('passed', '看看我的资产配置')
    expect(result.code, result.stderr).toBe(0)
