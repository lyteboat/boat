/**
 * `lyteboat studio` on the built launcher: it boots the studio profile (dsh
 * web with lyteboat's pages), serves the page behind the token exchange with
 * the Studio pages' browser bundle in its roster, answers the pages'
...
describe('lyteboat studio (built bin)', () => {
  ...
  it('serves dsh web with the Studio pages behind the token exchange, and stops cleanly on SIGTERM', async () => {
    const { home, workspace } = scratch.run('studio')
    const lyteboat = startLyteboat(['studio', '--agents', AGENTS, '--no-open', '--port', '0'], {
      cwd: workspace,
    ...
    try {
      ...
      await lyteboat.waitForStdout(/^lyteboat studio: agents echo$/mu, 30_000)
      // The token URL sets the session cookie and redirects to the page.
      const exchange = await fetch(launchUrl, { redirect: 'manual' })
      expect(exchange.status).toBe(303)
      const cookie = exchange.headers.get('set-cookie')?.split(';', 1)[0] ?? ''
      // Without the cookie the page is refused; with it the page lists the Studio pages' bundle.
      const anonymous = await fetch(new URL('/', launchUrl))
      expect(anonymous.status).toBe(401)
      const page = await fetch(new URL('/', launchUrl), { headers: { cookie } })
      expect(page.status).toBe(200)
      expect(await page.text()).toContain('@lyteboat/studio-pages/client.js')
  ...
  it('refuses an agent directory that does not exist, and a default agent no directory holds', async () => {
    ...
    const noDirectory = await runLyteboat(['studio', '--agents', missing, '--no-open'], { cwd: workspace, env })
    const noAgent = await runLyteboat(['studio', '--agents', AGENTS, '--agent', 'ghost', '--no-open'], { cwd: workspace, env })
