/**
 * Register the four finance tools through the tool policy: `auto`, so a tool
...
import { defineBucketDiagnosisTool } from './bucket-diagnosis-tool.ts'
...
export function registerFinanceTools(ctx: Context, deps: FinanceToolDeps): void {
  for (const definition of [defineAssetOverviewTool(deps), defineAllocationDiagnosisTool(deps), defineBucketDiagnosisTool(deps), defineLookupKnowledgeTool(deps)]) {
