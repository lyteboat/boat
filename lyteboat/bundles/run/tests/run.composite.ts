...
import { runComposition, type RunTarget } from './support/run-composition.ts'
...
describe('@lyteboat/run composition (in process, mock model)', () => {
  const scratch = createLyteboatScratch('run')
  ...
  it('answers one task through the real tool path, prints the answer, and exits 0', async () => {
    const result = await runComposition(['read the readme and report'], fresh('answer'), NO_TITLE_LLM)
  ...
  it('sends the model no session log: @lyteboat/host keeps dsh-base\'s session-log upload off', async () => {
    ...
    const result = await runComposition(['read the readme and report'], fresh('no-upload'), NO_TITLE_LLM)
    expect(result.code, result.stderr).toBe(0)
  ...
  it('rejects a missing task as a usage error without a model request', async () => {
    const before = mock.requests.length
    const result = await runComposition([], fresh('no-task'))
