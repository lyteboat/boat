/**
 * The agent directory `lyteboat run --agents <root> --agent <id>` composes from:
 ...
 * @module @lyteboat/run/agent-directory
 */
...
function readYaml(path: string, options?: LoadOptions): unknown {
  try {
    ...
    throw new Error(`lyteboat run: cannot read ${path}: ${error instanceof Error ? error.message : String(error)}`, { cause: error })
  }
}
...
function displayFields(dir: string): Pick<PresetDefinition, 'name' | 'description' | 'order'> {
  ...
  if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) throw new Error(`lyteboat run: ${path} must be a mapping of name, description, and order`)
  const { name, description, order } = raw as Record<string, unknown>
  if (name !== undefined && typeof name !== 'string') throw new Error(`lyteboat run: ${path}: name must be a string`)
  if (description !== undefined && typeof description !== 'string') throw new Error(`lyteboat run: ${path}: description must be a string`)
  if (order !== undefined && (typeof order !== 'number' || !Number.isFinite(order))) throw new Error(`lyteboat run: ${path}: order must be a finite number`)
  return {
    ...name === undefined ? {} : { name },
...
export function readAgentDefinition(id: string, dir: string): PresetDefinition {
  ...
  if (!Array.isArray(rows)) throw new Error(`lyteboat run: ${path} must be a list of plugin rows`)
