/**
 * Studio's command-line provider. dsh web's own startup row parses only the
 * web flags, and an unknown flag is a usage error there, so the studio bundle
 ...
 * rows read, and {@link LYTEBOAT_STUDIO_STARTUP_SERVICE}, which the agent
...
function command(): Command {
  return new Command()
    ...
    .addHelpText('after', `
Examples:
  lyteboat studio --agents ./agents               serve on the composed host and port and open the browser
  lyteboat studio --agents ./agents --no-open --port 8080
  lyteboat studio --agents ./agents --agent finance   new sessions run finance unless another agent is picked
`)
}
...
export function apply(ctx: Context): void {
  ...
  program.action(() => {
    ...
    ctx.provide(LYTEBOAT_STUDIO_STARTUP_SERVICE, { agentRoots, defaultAgent } satisfies LyteboatStudioStartupValues)
  })
