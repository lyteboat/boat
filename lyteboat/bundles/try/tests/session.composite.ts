/**
 * `lyteboat try --session-id` continues a stored session: dsh persistence
...
describe('lyteboat try --session-id (in process, scripted model)', () => {
  ...
  it('continues a routed session in place: the next turn derives the first and keeps the skill without injecting it again', async () => {
    ...
    const first = await tryComposition(['--agents', AGENTS, '--agent', 'routed', '看看我的资产'], target)
    expect(first.code, first.stderr).toBe(0)
    const id = printedSessionId(first.stderr)
    const before = model.requests.length

    const second = await tryComposition(['--agents', AGENTS, '--agent', 'routed', '--session-id', id, '那总额呢'], target)
  ...
  it('refuses an id that does not exist, a different agent, and --history', async () => {
    ...
    const unknown = await tryComposition(['--session-id', 'session-nope', 'hello'], target)
    ...
    expect(first.code, first.stderr).toBe(0)
    const id = printedSessionId(first.stderr)
    const plain = await tryComposition(['--session-id', id, 'hello'], target)
    ...
    const seeded = await tryComposition(['--history', join(FIXTURES, 'history', 'rounds.json'), '--session-id', id, 'hello'], target)
