 ...
 */
export function listEvalRuns(evalsDir: string, warn: (message: string) => void): StudioEvalRun[] {
  ...
  for (const id of readdirSync(evalsDir)) {
    ...
    if (!parsed.success) {
      warn(`studio-pages: eval run ${id} left out: ${file} is not a run.json this lyteboat reads (${parsed.error.issues[0]?.message ?? 'invalid'})`)
      continue
    }
