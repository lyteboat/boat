 ...
 * its skills are routed. No agent instance is created and nothing is written:
...
/** One skill of an agent, with its file where it has one. */
export interface AgentInspectorSkill {
  name: string
  description: string
  whenToUse?: string
...
export interface AgentInspectorSkillDetail extends AgentInspectorSkill {
  content: string
}

/** An agent's skills and how they are routed. */
export interface AgentInspectorSkills {
  skills: AgentInspectorSkill[]
...
export class AgentInspectorService extends Service {
  ...
  skills(agentId: string): Promise<AgentInspectorSkills | undefined> {
    return this.inScope(agentId, async (lookup) => {
      ...
      return {
        skills: await this.skillsIn(lookup),
        routing: {
          mode: settings.mode,
  ...
  private async skillsIn(lookup: SkillLookup): Promise<AgentInspectorSkill[]> {
...
function inspectedSkill(summary: SkillSummary, metadata: Readonly<Record<string, unknown>> | undefined): AgentInspectorSkill {
