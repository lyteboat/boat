 ...
 * goes on.
 * @module @lyteboat/web-pages/client/SessionTab
 */
...
import { errorStyle, fieldStyle, mutedStyle, preStyle, sectionTitleStyle } from './web-pages-styles.ts'

type SessionTabProps = PropsRuntime<'sidebar.right.pane.tab'> & InjectFace<WebPagesInjected>
 ...
 * @returns the tab.
 */
export function SessionTab({ sessionId, useProjection, pages }: SessionTabProps): ReactNode {
  ...
  return (
    <div style={{ padding: 12, overflow: 'auto', height: '100%', boxSizing: 'border-box', color: 'var(--dsw-alias-label-primary)', fontSize: 13 }}>
      <SendWithContext sessionId={sessionId} pages={pages} initialContext={request?.context ?? {}} />
...
interface SendWithContextProps extends WebPagesInjected {
  sessionId: string
  initialContext: { [key: string]: JsonValue }
}

function SendWithContext({ sessionId, pages, initialContext }: SendWithContextProps): ReactNode {
  ...
  const send = (): void => {
    ...
    setSending(true)
    pages.send({ sessionId, text, context: parsed })
