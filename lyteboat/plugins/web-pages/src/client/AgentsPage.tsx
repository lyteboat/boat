 ...
 * @module @lyteboat/web-pages/client/AgentsPage
 */
...
import type { WebPagesAgentsAnswer } from '../web-pages-endpoints.ts'
import { errorText, type WebPagesClient } from './web-pages-client.ts'
import { errorStyle, headerStyle, listStyle, mutedStyle, pageStyle, rowStyle, sectionTitleStyle, titleStyle } from './web-pages-styles.ts'

/** What the pages are handed at registration. */
export interface WebPagesInjected {
  readonly pages: WebPagesClient
}
 ...
 * @returns the page.
 */
export function AgentsPage({ pages }: PropsRuntime<'main'> & InjectFace<WebPagesInjected>): ReactNode {
  const [answer, setAnswer] = useState<WebPagesAgentsAnswer | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [reloading, setReloading] = useState(false)

  useEffect(() => {
    let live = true
    pages.agents().then(next => { if (live) setAnswer(next) }, (failure: unknown) => { if (live) setError(errorText(failure)) })
    return () => { live = false }
  }, [pages])
...
function AgentList({ answer }: { answer: WebPagesAgentsAnswer }): ReactNode {
  return (
    <>
