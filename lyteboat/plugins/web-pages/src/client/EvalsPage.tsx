 ...
 * @module @lyteboat/web-pages/client/EvalsPage
 */
...
import type { WebPagesEvalReportAnswer, WebPagesEvalsAnswer } from '../web-pages-endpoints.ts'
import type { WebPagesInjected } from './AgentsPage.tsx'
import { errorText } from './web-pages-client.ts'
import { errorStyle, headerStyle, listStyle, mutedStyle, pageStyle, preStyle, rowStyle, sectionTitleStyle, titleStyle } from './web-pages-styles.ts'
 ...
 * @returns the page.
 */
export function EvalsPage({ pages }: PropsRuntime<'main'> & InjectFace<WebPagesInjected>): ReactNode {
  const [runs, setRuns] = useState<WebPagesEvalsAnswer | null>(null)
  const [report, setReport] = useState<WebPagesEvalReportAnswer | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [generation, setGeneration] = useState(0)

  useEffect(() => {
    let live = true
    pages.evals().then(next => { if (live) setRuns(next) }, (failure: unknown) => { if (live) setError(errorText(failure)) })
    return () => { live = false }
  }, [pages, generation])

  const open = (run: string): void => {
    pages.evalReport(run).then(next => { setReport(next); setError(null) }, (failure: unknown) => { setError(errorText(failure)) })
  }
