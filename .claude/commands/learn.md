...
# 准备（已完成则跳过）
1. git fetch origin master，确认工作区是最新 master。回答第一行写“基于 master@短SHA”。
...
- 招熟（重点）：是什么、在哪里、怎么流动。给出三层架构图（Mermaid）、一次请求从入口到返回的主干调用链（不超过 15 步，标注所属层）、核心概念对照表，以及跑起来、跟踪请求、写一个业务 agent 的招式。
